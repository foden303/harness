#!/bin/bash
# config-utils.sh
# Utility for retrieving values from the harness config file
#
# Usage: source "${SCRIPT_DIR}/config-utils.sh"
#        plans_path=$(get_plans_file_path)

# Default path of the config file
CONFIG_FILE="${CONFIG_FILE:-.harness.config.yaml}"
...
read_i18n_language_from_config() {
  ...
  # Allow reading simple YAML from the standard template even without yq/PyYAML.
  if [ -z "$value" ]; then
    value="$(awk '
      function trim(s) {
        gsub(/^[[:space:]]+|[[:space:]]+$/, "", s)
...
get_legacy_plans_file_path() {
  ...
  # Search for Plans.md within the directory (case-insensitive)

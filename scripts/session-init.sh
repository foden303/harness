#!/bin/bash
# session-init.sh
# SessionStart Hook: initialization on session start
#
# Features:
...
#
# Output: emits info to hookSpecificOutput.additionalContext as JSON
...
# ===== Banner display (shown in the terminal via stderr) =====
VERSION=$(cat "$SCRIPT_DIR/../VERSION" 2>/dev/null || echo "unknown")
echo -e "\033[0;36m[harness v${VERSION}]\033[0m Session initialized" >&2
...
# Lightweight init for subagents (early return)
...
if [ "$AGENT_TYPE" = "subagent" ]; then
  cat <<EOF
{"hookSpecificOutput":{"hookEventName":"SessionStart","additionalContext":"[subagent] lightweight init complete"}}
EOF
  exit 0
fi
...
# Variable for accumulating output messages
OUTPUT=""
...
# Auto-repair broken symlinks from git clone on Windows
SYMLINK_INFO=""
...
# Clear the SSOT sync flag (on new session start)
# This flag is created when /sync-ssot-from-memory runs and
# is used to confirm SSOT sync before Plans.md cleanup
rm -f "${STATE_DIR}/.ssot-synced-this-session" 2>/dev/null || true

# Clear the work warning flag (on new session start)
...
# Generate a Harness session_id for the new session
NOW=$(date -u +%Y-%m-%dT%H:%M:%SZ)
HARNESS_SESSION_ID="session-$(date +%s)"

# Initialize session.json (if missing or in stopped state)
...
# Register self in active.json (so other sessions can discover it)
...
if [ -f "$PLANS_PATH" ]; then
  wip_count="$(count_matches "cc:wip\\|cc:WIP\\|pm:requested\\|pm:pending" "$PLANS_PATH")"
...
if [ -f "$TEMPLATE_TRACKER" ] && [ -f "$SCRIPT_DIR/../templates/template-registry.json" ]; then
  ...
  if [ ! -f "${STATE_DIR}/generated-files.json" ]; then
    ...
    # Update check (parse the JSON output)
    ...
    if command -v jq >/dev/null 2>&1; then
      ...
      if [ "$NEEDS_CHECK" = "true" ]; then
        ...
        # Files that need fresh installation
...
# ===== Step 6: Detect stale hook configuration =====
# Only detect hooks whose command path contains "harness" (exclude user-defined hooks)
OLD_HOOKS_INFO=""
...
if [ -f "$SETTINGS_FILE" ]; then
  if command -v jq >/dev/null 2>&1; then
    ...
    for event in "${PLUGIN_EVENTS[@]}"; do
      # Only when the event exists and its command contains "harness"
      if jq -e ".hooks.${event}" "$SETTINGS_FILE" >/dev/null 2>&1; then
        ...
        if echo "$COMMANDS" | grep -q "harness"; then
    ...
    if [ ${#OLD_HARNESS_EVENTS[@]} -gt 0 ]; then
      OLD_HOOKS_INFO="⚠️ Stale harness hook config detected: ${OLD_HARNESS_EVENTS[*]} → removal recommended with \`/harness-update\`"
    fi
  fi
fi

# ===== Build the output message =====
add_line "# [harness] Session initialization"
add_line ""

# SIMPLE mode warning (also emitted to additionalContext -- reuses check-simple-mode.sh warning text)
...
add_line "|---------|------|------|"
add_line "| \`cc:todo\` | not started | Impl (Claude Code) will run it |"
add_line "| \`cc:wip\` | in progress | Impl is implementing it |"
add_line "| \`cc:done\` | done | Impl finished, awaiting confirmation |"
add_line "| \`cc:blocked\` | blocked | waiting on a dependency task |"
add_line "| \`pm:requested\` | requested by PM | for 2-Agent operation |"
add_line "| \`pm:approved\` | confirmed by PM | for 2-Agent operation |"
add_line ""
add_line "> **Compatibility**: \`cc:TODO\` / \`cc:WIP\` / \`cc:done\` / \`pm:pending\` / \`pm:confirmed\` are also readable."

# ===== JSON output =====
# Claude Code's SessionStart hook accepts JSON-format hookSpecificOutput

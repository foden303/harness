...
#
# Usage: run automatically from PostToolUse hook (matcher="*")
# Input: stdin JSON (Claude Code hooks)
# Output:
#   - append JSONL to .claude/state/tool-events.jsonl (only when Phase0 log enabled)
#   - update .claude/state/tooling-policy.json (always, when LSP-related tool detected)
#
# Control: collect logs only when CC_HARNESS_PHASE0_LOG=1 is set
#          (disable after tool_name is settled to prevent log bloat)
#          LSP tracking always runs (does not depend on matcher "LSP", to avoid getting stuck)
...
# Acquire a lock (prefer flock, fall back to mkdir lock)
acquire_lock() {
  local lockfile="$1"
  local timeout=5
  local waited=0
...
# Extract required fields from JSON (prefer jq, fall back to python3)
TOOL_NAME=""
SESSION_ID=""
FILE_PATH=""
COMMAND=""

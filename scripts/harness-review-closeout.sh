...
# Lightweight harness-review closeout helper for the --quick closeout path.
#
...
Options:
  --dry-run             Print the review/test plan without running the tests.
...
# The review itself is performed by /harness-review in the host session, not by
# an external CLI. This helper reports the command to run and executes the
# declared test commands; it never claims a review happened that did not.
REVIEW_COMMAND="/harness-review ${BASE_REF}"
FALLBACK=""

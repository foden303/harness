...
# Utility to extract metadata from frontmatter
#
# Usage:
...
# Check whether frontmatter exists
has_frontmatter() {
  local file="$1"
  [ ! -f "$file" ] && return 1
  ...
  # Check whether _harness_version or _harness_template exists
...
get_file_version() {
  ...
  # 1. Try getting the version from frontmatter
  local version

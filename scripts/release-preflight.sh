...
Checks:
  ...
  - skill mirror drift when adapter paths, release claims, or explicit flags require it
...
release_claims_adapter_support() {
  ...
  printf '%s\n' "$unreleased" | grep -Eiq 'Codex|adapter|mirror|multi-harness|Hokage Core|capability matrix'
}
...
check_release_mirror_drift() {
  ...
  local missing=0
  for helper in scripts/sync-skill-mirrors.sh; do
    if [ ! -f "$helper" ]; then
  ...
  local diff_paths=("skills/")
  [ -d .agents/skills ] && diff_paths+=(".agents/skills/")

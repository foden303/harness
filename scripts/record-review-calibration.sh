...
# When review-result.json has calibration attached, append it to the training log.
#
...
jq -c -n \
  ...
  ($src[0] // {}) as $in
  | ($rr[0] // {}) as $rv
  # critical_count: sum the same sources as write-review-result.sh normalization
  ...
  # major_count: sum the same sources as write-review-result.sh normalization

...
# Dry-run inventory of Harness skill origins across Claude/Codex paths.
set -euo pipefail
...
usage() {
  cat <<'EOF'
Usage: diagnose-harness-skill-duplication.sh [--host claude|codex|all] [--profile clean|compatibility] [--json]
...
recommend_primary() {
  case "$HOST" in
    claude) echo "claude-plugin-ssot or harness marketplace plugin" ;;

...
#
# Behavior:
# - Disabled by default via .harness.config.yaml
...
if [ "$QUALITY_TSC" = "true" ]; then
  if [ "$QUALITY_MODE" = "run" ]; then
    if run_tsc; then
      append_feedback "🧪 tsc --noEmit: done"
    else
      append_feedback "🧪 tsc --noEmit: not run (tsconfig/tsc not found)"
    fi
  else
...
if [ "$QUALITY_CONSOLE_LOG" = "true" ]; then
  if [ -f "$FILE_PATH" ]; then
    ...
    if [ "$CONSOLE_LOG_COUNT" -gt 0 ]; then
      append_feedback "⚠️ Found ${CONSOLE_LOG_COUNT} console.log occurrences"
    fi
  fi
fi

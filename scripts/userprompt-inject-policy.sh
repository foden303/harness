...
# extract a value from JSON (prefer jq, otherwise python3)
json_get() {
  local json="$1"
  local key="$2"
  local default="${3:-}"
...
with open('$file', 'r') as f:
    data = json.load(f)
# Simplified update (only handles incrementing prompt_seq)
...
# semantic/literal decision (keyword-based)
INTENT="literal"
SEMANTIC_KEYWORDS="rename"
...
if command -v jq >/dev/null 2>&1; then
  ...
  # without jq, do a minimal update via the python fallback
...
# reset the LSP-used flag in tooling-policy.json (this is a new prompt)
...
# ===== detect Work mode and inject the one-time mandatory harness-review warning =====
# as a safety net for when session-resume.sh does not fire after compact,
# inject the warning exactly once on UserPromptSubmit
# backward compat: prefer work-active.json, fall back to ultrawork-active.json
...
if [ -f "$WORK_FILE" ] && [ ! -f "$WORK_WARNED_FLAG" ] && command -v jq >/dev/null 2>&1; then
  ...
  if [ "$REVIEW_STATUS" != "passed" ]; then
    ...
    # create a flag so we warn only once
...
if [ "$INTENT" = "semantic" ]; then
  if [ "$LSP_AVAILABLE" = "true" ]; then
    ...
    # LSP not installed: recommendation only (does not deny)
    INJECTION="${INJECTION}$(policy_msg lsp_recommendation)"
  fi
fi

# ===== inject the Unified Memory Resume Pack (inject the SessionStart-fetched context once) =====
RESUME_BUSY=0

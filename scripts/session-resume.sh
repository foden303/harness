...
#
# Called automatically when Claude Code's /resume command runs, and
...
echo -e "\033[0;36m[harness v${VERSION}]\033[0m Session resumed" >&2
...
# ===== Harness state directory (unified on repo root) =====
...
CONFIG_FILE="${CONFIG_FILE:-${REPO_ROOT}/.harness.config.yaml}"
...
# Method 1: look up from the session mapping
...
# Method 2: auto-restore the latest stopped session (when no mapping exists)
...
# Method 3: initialize fresh when there is nothing to restore
if [ "$RESTORED" = "false" ]; then
...
# ===== Save the mapping to the Claude Code session_id =====
...
# ===== Clear the SSOT sync flag (on new/restored session start) =====
...
# Clearing it on restore re-displays the warning on the first prompt after restore
...
if [ -f "$PLANS_PATH" ]; then
  wip_count="$(count_task_matches "(cc:(wip|WIP)|pm:(requested))" "$PLANS_PATH")"
...
# ===== Detect active_skill (check whether a skill needs restarting) =====
ACTIVE_SKILL_INFO=""
...
# ===== Work mode detection and re-injection of mandatory harness-review =====
WORK_INFO=""
WORK_FILE="${STATE_DIR}/work-active.json"
# Backward compatibility: fall back to ultrawork-active.json if work-active.json is missing
if [ ! -f "$WORK_FILE" ]; then
...
if [ -f "$WORK_FILE" ] && command -v jq >/dev/null 2>&1; then
  ...
  case "$REVIEW_STATUS" in
    "passed")
      WORK_INFO="⚡ **work mode in progress** (started: ${STARTED_AT})\n   ✅ review_status: passed → completion is allowed"
      ;;
    "failed")
      WORK_INFO="⚡ **work mode in progress** (started: ${STARTED_AT})\n   ❌ review_status: failed → after fixing, re-run /harness-review"
      ;;
    *)
...
add_line "# [harness] Session restore"
add_line ""

case "$RESTORE_METHOD" in
  "mapping")
    add_line "✅ Restored session state (found via mapping)"
  ...
  "latest")
    ...
    ;;
  "new")
    add_line "ℹ️ No session to restore, so initialized a new one"
    add_line "   Harness Session: ${RESTORED_SESSION_ID}"
    ;;
esac
...
# Add active_skill restart instructions (displayed with top priority)

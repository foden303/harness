...
function findWorktreeStateDirs(repoRoot) {
  // In Breezing, Workers/Reviewers run in separate git worktrees, so
  ...
  try {
    ...
    // Ignore cases where git worktree list fails
  }
  return dirs;
}
...
function buildReport(stateDir) {
  ...
  // Combine the main and worktree statuslines
  ...
  for (const record of traces) {
    ...
    accumulate(buckets, role, (bucket) => {
      ...
      // duration may already be counted by the statusline, so
      // when statusline_count > 0, skip adding duration from the trace
      // (the statusline has finer, more accurate sampling granularity)
  ...
  // Combine the main and worktree usage
  ...
  // Merge the worktree usage into the first usage
  ...
  // Combine the main and worktree events

...
#
# Purpose:
#   Extract the counts and lists of cc:todo / cc:wip / cc:done from the
#   Plans.md task table, and generate snapshot JSON for the harness-progress skill.
#
# Usage:
...
#   Status is cc:todo / cc:wip / cc:done [hash] (legacy cc:TODO / cc:WIP / cc:done also readable)
#   pm:* is out of scope for this snapshot
#
# Optional: --state-file
#   Read elapsed_minutes / cost_so_far_usd etc. from .claude/state/session-cost.json
#   (all 0 if absent)
...
# Use the text up to the first "。" as a one-line summary for title
...
with open(PLANS_PATH, "r", encoding="utf-8") as f:
    for line in f:
        ...
        # Shorten title: cut at the first "。", otherwise up to 80 chars
        ...
        elif status.startswith("cc:done") or status.startswith("cc:done"):

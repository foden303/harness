...
#
# Usage: auto-run from the PostToolUse hook
...
# normalize to a project-relative path if possible (cross-platform)
...
if command -v jq &> /dev/null; then
  ...
  TEMP_FILE=$(mktemp 2>/dev/null) || {
    # if mktemp fails, skip silently (do not interrupt; this is a PostToolUse hook)
    exit 0
  }
  # guarantee cleanup
...
# ==============================================================================
# if a Write/Edit occurs during /work, reset review_status to pending
# this ensures that a re-review is always required after code changes

...
# Resolve the Plans.md path (fallback order: git root -> PROJECT_ROOT -> cwd)
_GIT_ROOT="$(git rev-parse --show-toplevel 2>/dev/null || true)"
PLANS_MD="${PROJECT_ROOT:-${_GIT_ROOT:-$(pwd)}}/Plans.md"

# Extract task info from Plans.md
# Assumed v2 format: | Task | Content | DoD | Depends | Status |
TASK_CONTENT=""
TASK_DOD=""
TASK_DEPENDS=""
...
if [ -f "$PLANS_MD" ]; then
  ...
  if echo "$TASK_INPUT" | grep -qE '^#?[0-9]+(\.[0-9]+)*$'; then
    ...
    # Parse the table row: | number | content | DoD | Depends | Status |
    TASK_ROW=$(grep -E "^\|[[:space:]]*${TASK_ID_PATTERN}[[:space:]]*\|" "$PLANS_MD" 2>/dev/null || true)
    if [ -n "$TASK_ROW" ]; then
      # Extract columns split by pipe
  ...
  # If not found by task ID, do a keyword search on the description
  if [ -z "$TASK_CONTENT" ]; then
    # Extract the line closest to the task description in Plans.md (table row)
...
# If nothing could be retrieved from Plans.md, analyze the task input itself
if [ -z "$TASK_CONTENT" ]; then
...
if [ -n "$TASK_DEPENDS" ]; then
  # Number of dependent tasks in the Depends column (dotted ID: 34.1.1, simple ID: #123, comma-separated, etc.)
  # Count dotted IDs first, then count the rest as simple numeric IDs
...
# 3. Keyword check -> +1 (add if 1+ matches, no duplicates)
...
# 4. Multiple DoD conditions (2+) -> +1
if [ -n "$TASK_DOD" ]; then
  # Count conditions separated by semicolon or comma (delimiter count + 1 = condition count)

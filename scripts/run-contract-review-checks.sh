...
# Run the runtime_validation defined in sprint-contract.json in order,
...
# Output with an absolute path (so the Lead can resolve it even when called from a worktree)
...
# Instead of returning APPROVE/REQUEST_CHANGES with no checks,

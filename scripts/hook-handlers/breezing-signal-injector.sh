...
# In the UserPromptSubmit hook, reads unconsumed signals from breezing-signals.jsonl
...
# Lines where consumed_at is null or missing are treated as unconsumed
UNCONSUMED_SIGNALS=""
...
while IFS= read -r signal_line; do
  ...
  case "${signal_type}" in
    ...
    escalation_required)
      ...
      SYSTEM_MESSAGE="${SYSTEM_MESSAGE}[SIGNAL:escalation_required] Task #${task_id} requires escalation. Reason: ${reason}\n"
      ;;
    *)
...
# Atomic update: add consumed_at to a new file and overwrite
...
if [ "${_lock_acquired}" -eq 1 ]; then
  ...
  if command -v jq >/dev/null 2>&1; then
    # Rewrite all signals with consumed_at added
    while IFS= read -r line; do
      [ -z "${line}" ] && continue
      consumed="$(printf '%s' "${line}" | jq -r '.consumed_at // "null"' 2>/dev/null)"
      if [ "${consumed}" = "null" ]; then

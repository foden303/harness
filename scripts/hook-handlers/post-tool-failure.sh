...
STALENESS_THRESHOLD=60  # Seconds. Reset if more than 60s elapsed since the last failure
...
if [ -f "${COUNTER_FILE}" ]; then
  ...
  # Reset if enough time has elapsed since the last failure (not consecutive)
...
if [ "${CURRENT_COUNT}" -ge 3 ]; then
  # Build an escaped error message
  if command -v jq >/dev/null 2>&1; then
    jq -nc \
      --arg tool "${TOOL_NAME}" \
      --arg error "${ERROR_MSG}" \

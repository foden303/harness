...
# Records the denial event in telemetry and, in Breezing mode, notifies the Lead.
...
# Returning retry: true tells the model that a retry is possible
_notification_text="[PermissionDenied] Worker tool ${TOOL_NAME} was denied in auto mode. Reason: ${DENIED_REASON}. Consider an alternative approach or approve it manually if needed."
_is_worker=false

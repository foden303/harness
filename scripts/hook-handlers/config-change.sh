...
#
# Fires when a config file changes. Records to the timeline only when breezing is active.
...
# Check that detect_project_root is defined before calling it
...
if [ ! -t 0 ]; then
  if [ -n "$_TIMEOUT" ]; then
    ...
    # No timeout available: use dd to enforce a byte-count limit (POSIX standard)
...
if [ "$BREEZING_ACTIVE" = true ] && [ -n "$PAYLOAD" ]; then
  ...
  # Normalize file_path to a repo-relative path (hides username, etc.)

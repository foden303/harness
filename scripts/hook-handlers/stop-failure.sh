...
# Records the error info in the log and, in Breezing mode, attempts to notify the Lead.
#
...
if [ "${ERROR_CODE}" = "429" ]; then
  ...
  # Build JSON with SESSION_ID safely escaped by jq/python3
  _sys_msg=""
  if command -v jq > /dev/null 2>&1; then
    _sys_msg="$(jq -nc --arg sid "${SESSION_ID}" \
      '{systemMessage: ("[StopFailure] Worker " + $sid + " stopped due to rate limit (429). The Breezing Lead should attempt auto-resume after exponential backoff.")}')"
  elif command -v python3 > /dev/null 2>&1; then
    _sys_msg="$(python3 -c "
import json, sys
print(json.dumps({'systemMessage': '[StopFailure] Worker ' + sys.argv[1] + ' stopped due to rate limit (429). The Breezing Lead should attempt auto-resume after exponential backoff.'}, ensure_ascii=False))
" "${SESSION_ID}" 2>/dev/null)" || _sys_msg=""
  fi
  [ -n "${_sys_msg}" ] && echo "${_sys_msg}"
fi

# Also emit to stderr (for debugging)

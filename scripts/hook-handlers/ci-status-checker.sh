...
# In PostToolUse (Bash matcher), asynchronously checks CI status after git push / gh pr
# On detecting CI failure, injects a message recommending ci-cd-fixer spawn via additionalContext
#
...
# === Determine whether it is a git push / gh pr command ===
...
# CI check polls for up to 60 seconds (only when the gh command exists)
...
check_ci_status_async() {
  ...
  while [ "${elapsed}" -lt "${max_wait}" ]; do
    ...
    # On CI failure, write out the signal file
...
# Run the CI check in the background (does not block the hook)
check_ci_status_async &
disown 2>/dev/null || true

# === Check the most recent CI failure signal and inject additionalContext ===
ADDITIONAL_CONTEXT=""
SIGNALS_FILE="${STATE_DIR}/breezing-signals.jsonl"
...
if [ -f "${SIGNALS_FILE}" ]; then
  ...
  if [ -n "${_recent_failure}" ]; then
    ...
    ADDITIONAL_CONTEXT="[CI failure detected]\nCI status: ${_failure_conclusion}\nTrigger command: ${BASH_CMD}\n\nRecommended action: run /breezing or spawn the ci-cd-fixer agent to auto-repair the CI failure.\n  Example: ask ci-cd-fixer, 'CI failed. Check the logs and fix it.'"
  fi
fi

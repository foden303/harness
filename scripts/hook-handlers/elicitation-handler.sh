...
# During a Breezing session (background Worker/Reviewer), auto-skips since interaction is impossible
#
...
# JSONL rotation (truncate to 400 lines when exceeding 500 lines)
rotate_jsonl() {
  local file="$1"
  local _lines
...
# === During a Breezing session, auto-skip elicitation ===
# Because a background Worker/Reviewer cannot interact with the UI
...
# === Normal session: pass through as-is (the user responds interactively) ===

...
# === Timeline recording (safe JSON built with jq -nc) ===
ensure_state_dir
TS="$(get_timestamp)"
...
if [ -f "${BREEZING_ACTIVE}" ]; then
  # Get the session ID (for signal session scoping)
  SESSION_ID=""
  if command -v jq >/dev/null 2>&1; then
...
" "${BREEZING_ACTIVE}" 2>/dev/null)" || SESSION_ID=""
  ...
  if [ -f "${TIMELINE_FILE}" ] && [ -n "${CURRENT_BATCH_IDS}" ]; then
    ...
    # Fallback: count everything when batch IDs are unavailable
    COMPLETED_COUNT="$(grep -Fc '"event":"task_completed"' "${TIMELINE_FILE}" 2>/dev/null)" || COMPLETED_COUNT=0
  fi

  # Signal dedup helper: checks for duplicates at session scope
  # With SESSION_ID, dedup per session (AND within the same record); otherwise dedup globally
  _signal_exists() {
    local sig_type="$1"
    if [ -n "${SESSION_ID}" ] && [ -f "${SIGNALS_FILE}" ]; then
      # Whether a record containing both signal and session_id on the same line exists
...
" "${sig_type}" "${SESSION_ID}" "${completed}" "${total}" "${ts}" 2>/dev/null
  ...
  # -ge handles overshooting the threshold (e.g. simultaneous completions); the already-emitted check prevents duplicates
  # Rounding up prevents the threshold from triggering too early
  ...
  if [ "${TOTAL_TASKS}" -gt 0 ] 2>/dev/null; then
    ...
    # Round up: (n * 60 + 99) / 100 prevents early triggering from truncation
...
# Check the auto-test-runner result file; escalate with exit 2 when not run / failed
TEST_RESULT_FILE="${STATE_DIR}/test-result.json"
QUALITY_GATE_FILE="${STATE_DIR}/task-quality-gate.json"

# Update the failure count per task ID
...
check_test_result() {
  ...
  if [ ! -f "${TEST_RESULT_FILE}" ]; then
    return 0  # Skip when not run (some projects do not need tests)
  fi
...
" "${TEST_RESULT_FILE}" 2>/dev/null)" || status=""
  ...
  # Detect a failure when status=failed
...
if ! check_test_result; then
  ...
  # Escalate after 3 consecutive failures (D21 automation: exit 0 + stderr output)
...
" "${TEST_RESULT_FILE}" 2>/dev/null)"
    ...
    # Cause classification (inferred from keywords in the test output)
    if echo "${_last_output}" | grep -qi "syntax\|SyntaxError\|parse error\|unexpected token"; then
      _failure_category="syntax_error"
      _recommended_action="Fix the syntax error. Check the code syntax."
      ...
      _recommended_action="A permission error occurred. Check the file permissions."
    else
      _failure_category="runtime_error"
      _recommended_action="A runtime error occurred. Check the test output in detail."
    fi
    ...
    # Emit the escalation report to stderr
    {
      echo ""
      echo "=========================================="
      echo "[ESCALATION] Detected 3 consecutive failures - stopping the auto-fix loop"
      echo "=========================================="
      ...
      echo "  Manual intervention required. The loop will end after recording the escalation."
      echo "=========================================="
      echo ""
    } >&2 2>/dev/null || true
    ...
    _fix_dod="$(sanitize_inline_text "Resolve the failure category (${_failure_category}) and make the latest test/CI pass")"
    _proposal_json=""
    if command -v jq >/dev/null 2>&1; then
...
Warning: failed to save the proposal. Add it to Plans.md manually."
    ...
    # Approve the hook with exit 0 while presenting a proposal that can be approved next turn
    if command -v jq >/dev/null 2>&1; then
      jq -nc \
...
# === Webhook notification helper (a function so it can be called on every exit path) ===
_fire_webhook() {

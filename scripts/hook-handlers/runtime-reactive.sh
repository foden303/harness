...
# - FileChanged: detect changes to Plans / rules / settings and prompt a re-read
  ...
  FileChanged)
    case "${FILE_PATH}" in
      ...
      AGENTS.md|*/AGENTS.md|CLAUDE.md|*/CLAUDE.md|.claude/rules/*|*/.claude/rules/*|hooks/hooks.json|*/hooks/hooks.json|.claude-plugin/settings.json|*/.claude-plugin/settings.json)
        MESSAGE="Work rules or Harness settings were updated. Proceed with the latest rules in mind for the next operation."
        ;;
    esac
    ;;
  CwdChanged)
    MESSAGE="The working directory changed. If you moved to another repository or worktree, re-check AGENTS.md, Plans.md, and local rules."
    ;;
esac

...
# Records events such as permission_prompt, idle_prompt, auth_success
#
...
# Recording to the log enables post-hoc analysis
...
# elicitation_dialog: input request from an MCP server (v2.1.76+)
# Background Workers cannot respond to Elicitation forms
# Already auto-skipped by the Elicitation hook, but also recorded in the notification log
...
# For notifications that should grab the operator's attention, like permission_prompt / elicitation_dialog,

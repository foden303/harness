...
# When WIP tasks exist, restores the state saved by PreCompact and injects it as a systemMessage
# If a structured handoff artifact exists, prefer it to re-inject high-signal key points
#
...
# Prefer restoring the structured handoff artifact
...
# Restore the WIP info saved by PreCompact so the model keeps track of task state after compaction
SYSTEM_MESSAGE=""

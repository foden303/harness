...
# In the UserPromptSubmit hook, notifies about pending fix proposals and,
# on receiving an approve/reject prompt, applies them to Plans.md.
...
if path_has_symlink_component_within_root "${STATE_DIR}" "${PROJECT_ROOT}" || [ -L "${PENDING_FIX_PROPOSALS_FILE}" ]; then
  emit_system_message "⚠️ Aborted: the fix proposal state path is a symlink."
  exit 0
fi

if [ -e "${PLANS_PATH}" ] && path_has_symlink_component_within_root "${PLANS_PATH}" "${PROJECT_ROOT}"; then
  emit_system_message "⚠️ Cannot apply the fix proposal: the Plans.md path is a symlink."
  exit 0
fi
...
if [ -n "${ACTION}" ] && [ -z "${TARGET_ID}" ] && [ "${PENDING_COUNT}" != "1" ]; then
  emit_system_message "⚠️ There are ${PENDING_COUNT} pending fix proposals. Use approve fix <task_id> or reject fix <task_id> to specify which one."
  exit 0
fi
...
if [ "${SELECT_STATUS}" -eq 2 ] && [ -n "${TARGET_ID}" ]; then
  emit_system_message "⚠️ Specified fix proposal not found: ${TARGET_ID}"
  exit 0
fi
...
if [ "${ACTION}" = "approve" ]; then
  ...
  case "${APPLY_RESULT}" in
    applied|already_present)
      ...
      ;;
    plans_missing)
      emit_system_message "⚠️ Could not apply the fix proposal. Plans.md not found."
      exit 0
      ;;
    *)
      emit_system_message "⚠️ Failed to apply the fix proposal. Source task ${SOURCE_TASK_ID} not found in Plans.md."
      exit 0
      ;;
  esac
fi
...
REMINDER="[FIX PROPOSAL] Pending fix task proposals (${PENDING_COUNT})\n"

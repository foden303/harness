...
# Output: JSON-formatted project analysis results
#
...
# ================================
# 4. Existing Claude config detection
# ================================
...
detect_existing_setup() {
  ...
  if [ -f ".harness-version" ]; then
    version=$(grep "^version:" .harness-version 2>/dev/null | cut -d' ' -f2 || echo "unknown")
  fi
...
# ================================
# 6. Important patterns detection (keyword-based)
# ================================
...
detect_important_patterns() {
  ...
  # Security-related
  if [ -f "SECURITY.md" ] || grep -riqE "security" $search_files 2>/dev/null; then
    patterns+=("security")
  fi

  # Testing emphasis
  if grep -riqE "test coverage|coverage.*%|must have tests|require.*test" $search_files 2>/dev/null; then
    patterns+=("testing-required")
  fi

  # Accessibility
  if grep -riqE "accessibility|a11y|wcag|aria" $search_files package.json 2>/dev/null; then
    patterns+=("accessibility")
  fi

  # Performance
  if grep -riqE "performance|core web vitals|lighthouse|N\+1" $search_files 2>/dev/null; then

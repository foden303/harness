...
COMPANION_BIN="${HARNESS_ADVISOR_COMPANION:-${PROJECT_ROOT}/scripts/advisor-companion.sh}"
...
CONFIG_FILE="${PROJECT_ROOT}/.harness.config.yaml"
source "${CONFIG_UTILS}"
ensure_advisor_state_files

if [ -z "${MODEL}" ]; then
  MODEL="$(get_advisor_claude_model)"
fi

#!/bin/bash
# session-monitor.sh
# Collect and display project state on session start
#
...
# Get the Plans.md path accounting for the plansDirectory setting
...
# Compute relative time (from seconds to "X minutes ago", "X hours ago", etc.)
relative_time() {
  local seconds=$1
...
if [ -f "$PLANS_FILE" ]; then
  ...
  # The English marker family is canonical; Japanese is treated as a compatible synonym
  PENDING_COUNT=$(( $(count_tasks "pm:requested") + $(count_tasks "pm:pending") ))
  COMPLETED_COUNT=$(( $(count_tasks "cc:done") + $(count_tasks "cc:done") ))
else
  PLANS_EXISTS="false"
  PLANS_MODIFIED="0"
  WIP_COUNT="0"
...
# Read the Orchestration settings (simple parse)
ORCH_MAX_RETRIES="3"
ORCH_BACKOFF="10"
...
# If an existing session has not ended, attempt to resume it
EXISTING_SESSION_ID=""
EXISTING_ENDED_AT=""
...
# Generate the skills index (name + description)
SKILLS_INDEX="[]"
if [ -d "skills" ]; then
...
# Map LSP availability by extension (simplified)
LSP_BY_EXT="{}"
if [ "$LSP_AVAILABLE" = "true" ]; then
  # If official LSP plugins are installed, map their supported extensions

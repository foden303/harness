...
# Actually run the tests and write out the result file (HARNESS_AUTO_TEST=run mode)
run_tests() {
    local test_cmd="$1"
    ...
    # Determine the exec command (narrow down when related tests exist)
    ...
    # Capture the output (max 200 lines)
...
main() {
    ...
    if [ "${HARNESS_AUTO_TEST:-}" = "run" ]; then
        ...
        # Print a result summary to stderr (for hooks log)
        RESULT_FILE="${STATE_DIR}/test-result.json"
        if [ -f "$RESULT_FILE" ]; then
            ...
            # Notify Claude of the test result via additionalContext
    ...
    cat > "${STATE_DIR}/test-recommendation.json" << EOF
{
  ...
  "recommendation": "Running the tests is recommended"
}
EOF

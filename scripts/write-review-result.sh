...
jq -n   --slurpfile src "$INPUT_FILE"   --slurpfile browser "$BROWSER_SLURP_FILE"   --arg generated_at "$(date -u +%Y-%m-%dT%H:%M:%SZ)"   --arg commit_hash "$COMMIT_HASH" '
  ...
  # companion verdict normalization: approve→APPROVE, needs-attention→REQUEST_CHANGES
  ...
  # companion findings[] → gaps[] mapping (input received as an argument)
...
# if there are blocking gaps, force the verdict to REQUEST_CHANGES

...
#   (1) opt-in disabled (default) → handler is a no-op
#   (2) opt-in enabled + redact use → correctly redacts + records audit
#   (3) JSON-contract tool (Read/Grep/Bash) → skip to prevent mixing
#   (4) audit log is append-only and preserves before/after
#
# Also explicitly verify failure uses (tampering):
#   (5) the handler does not permit hiding evidence in review / test output
#       (since the handler is "allowlist-based", adding a new rule requires explicit registration)
...
# (5) since hiding test output / review evidence is not in the handler's allowlist,
#     pin via source inspection that no corresponding rule is registered.
...
# (6) the handler being explicitly opted-in via HARNESS_OUTPUT_GOVERNANCE_ENABLE=1
#     must be written literally in the code

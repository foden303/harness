...
# - Extract `skill:` from workflows/**/*.yaml
...
skill_exists() {
  ...
  # exists as a directory under skills/*/$skill_name/ (nested skill)
  ...
  # defined via frontmatter name: (including reference skills)
  ...
  for skill in $SKILLS; do
    ...
    if skill_exists "$skill"; then
      ...
      if [ "$STRICT_MODE" = true ]; then
        log_fail "skill: $skill" "skill '$skill' not found under skills/ or agents/"
      else
        log_warn "skill: $skill - not implemented (planned for the future)"
      fi
    fi
  done
done
...
echo "--- Verify agent definitions ---"

AGENT_FILES=$(find "$PROJECT_ROOT/agents" -name "*.md" 2>/dev/null || true)

if [ -n "$AGENT_FILES" ]; then
  AGENT_COUNT=$(echo "$AGENT_FILES" | wc -l | tr -d ' ')
  log_pass "agents/ has $AGENT_COUNT agent definitions"
else
  log_warn "agents/ has no agent definitions"
fi

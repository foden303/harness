...
# by Phase 52's upstream-update skill check in
...
if [ -z "${HARNESS_ROOT}" ]; then
  ...
  echo "  to run this test locally, clone harness-mem alongside harness:"
...
STOP_PAYLOAD='{"session_id":"claude-session-1","summary_mode":"standard","last_assistant_message":"1. Problem: opening a new session loses the context of the previous conversation 2. Decision: always show the continuity briefing on the first turn 3. Next steps: align adapter delivery for Claude"}'

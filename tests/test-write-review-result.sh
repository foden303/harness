...
cat > "${TMP_DIR}/input.json" <<'EOF'
{
  ...
  "task": {
    "id": "32.0.2",
    "title": "unify the review artifact"
  },
  "critical_issues": [],
  "major_issues": [],
...
cat > "${TMP_DIR}/calibration-input.json" <<'EOF'
{
  ...
  "calibration": {
    ...
    "source": "post-review",
    "notes": "caught a missed major later",
    "prompt_hint": "verify the basis for major in the diff",
    "few_shot_ready": true
  },
  "gaps": [
    {
      "severity": "major",
      "issue": "a key diff check was missing",
      "suggestion": "re-check the diff"
    }
  ],
  "followups": ["revise the reviewer prompt"]
}
EOF

...
cat > "${TMP_DIR}/calibration-input-1.json" <<'EOF'
{
  ...
  "calibration": {
    ...
    "source": "manual",
    "notes": "Blocked a diff that was not serious",
    "prompt_hint": "Keep it minor when there is no evidence",
    "few_shot_ready": true
  },
  "gaps": [
    {
      "severity": "major",
      "issue": "Over-blocked an improvement suggestion",
      "suggestion": "Treat it as minor"
    }
  ]
}
EOF
...
cat > "${TMP_DIR}/calibration-input-2.json" <<'EOF'
{
  ...
  "calibration": {
    ...
    "notes": "Missed a broken UI layout",
    "prompt_hint": "Look at the screenshot diff first",
    "few_shot_ready": true
  },
  "gaps": [
    {
      "severity": "major",
      "issue": "Missed a broken screen layout",
      "suggestion": "Add a browser snapshot"
    }
  ]
}
EOF
...
jq -e '
  ...
  ([.entries[].prompt_hint] | index("Look at the screenshot diff first")) != null

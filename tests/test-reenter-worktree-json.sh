#!/usr/bin/env bash
# Verify that reenter-worktree.sh prints only JSON to stdout
...
PY
then
  echo "FAIL: stdout is not parseable as JSON or does not match the expected value"
  echo "stdout:"
  cat "${STDOUT_FILE}"
  exit 1
fi

if ! grep -q "EnterWorktree path re-entry check" "${STDERR_FILE}"; then
  echo "FAIL: human-facing guidance was not emitted to stderr"
  echo "stderr:"
  cat "${STDERR_FILE}"
  exit 1
fi

...
#
# What is verified:
#   (1) The 3 trigger types (human / model / skill-chain) are recorded distinctly
#   (2) opt-out (HARNESS_SKILL_TELEMETRY_DISABLE=1) prevents writing
#   (3) skill_telemetry_exclude excludes an individual skill
#   (4) The ledger is append-only (no deletion / overwrite)
#   (5) session_id is truncated to a 12-char prefix (privacy)
...
# (1) Feed the 3 triggers in order
...
# Remove the exclusion and confirm it is written
...
echo "PASS: test-skill-trigger-telemetry.sh (Phase 62.2.3) — all 5 checks PASS"

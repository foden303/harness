...
# callers (`scripts/auto-checkpoint.sh`) still expect
...
if [ -z "${HARNESS_MEM_CLIENT_SRC}" ]; then
  ...
  echo "  to run locally, clone harness-mem alongside harness:"
...
mkdir -p "${TMP_DIR}/harness/scripts/lib"
mkdir -p "${TMP_DIR}/harness/scripts/hook-handlers"
mkdir -p "${TMP_DIR}/harness-mem/scripts/hook-handlers"

cp "${ROOT_DIR}/scripts/lib/harness-mem-bridge.sh" "${TMP_DIR}/harness/scripts/lib/harness-mem-bridge.sh"
cp "${ROOT_DIR}/scripts/hook-handlers/memory-session-start.sh" "${TMP_DIR}/harness/scripts/hook-handlers/memory-session-start.sh"
cp "${HARNESS_MEM_CLIENT_SRC}" "${TMP_DIR}/harness/scripts/harness-mem-client.sh"
...
chmod +x \
  "${TMP_DIR}/harness/scripts/hook-handlers/memory-session-start.sh" \
  "${TMP_DIR}/harness/scripts/harness-mem-client.sh" \
  "${TMP_DIR}/harness-mem/scripts/hook-handlers/memory-session-start.sh" \
  "${TMP_DIR}/harness-mem/scripts/harness-mem-client.sh"

wrapper_output="$(cd "${TMP_DIR}/harness" && ./scripts/hook-handlers/memory-session-start.sh)"
client_output="$(cd "${TMP_DIR}/harness" && ./scripts/harness-mem-client.sh health)"

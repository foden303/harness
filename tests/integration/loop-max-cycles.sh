#!/bin/bash
# loop-max-cycles.sh
# Verify harness-loop stops without exceeding the maximum cycle count
...
grep -q 'max cycles reached (3)' "${LOG_FILE}" || {
  echo "max cycles reached notice not found"
  exit 1
}

grep -q 'task-4' "${LOG_FILE}" && {
  echo "the 4th task was processed"
  exit 1
}

[ ! -d "${LOCK_DIR}" ] || {
  echo "lock directory still present"
  exit 1
}

#!/bin/bash
# Verify that usage tracking hooks do not emit record-usage noise on stdout
...
usage_output="$(cd "${TMP_DIR}" && printf '%s' '{"tool_name":"Skill","tool_input":{"skill":"harness:memory"}}' | bash "${USAGE_TRACKER_SCRIPT}")"

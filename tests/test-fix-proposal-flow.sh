#!/bin/bash
# Verify TaskCompleted 3-strike -> pending fix proposal -> approve fix is reflected in Plans.md
...
echo "${ambiguous_output}" | grep -q 'to specify which one' || {
...
echo "${symlink_output}" | grep -q 'failed to save the proposal' || {
...
echo "${plan_symlink_output}" | grep -q 'the Plans.md path is a symlink' || {

...
[Unreleased]: https://github.com/foden303/harness/compare/v1.2.3...HEAD
EOF

  local bump_output="$TMP_DIR/sync-bump.txt"
  (cd "$repo" && bash "$PROJECT_ROOT/scripts/sync-version.sh" bump patch > "$bump_output")

  assert_contains "$bump_output" "Updated VERSION (patch): 1.2.3 → 1.2.4"
  assert_contains "$bump_output" "Updated marketplace.json: metadata.version: 1.2.3 → 1.2.4"
  assert_contains "$bump_output" "Updated marketplace.json: plugins[1](fixture-extra).version: 1.2.3 → 1.2.4"
  ...
  assert_contains "$repo/CHANGELOG.md" "[1.2.4]: https://github.com/foden303/harness/compare/v1.2.3...v1.2.4"
}
...
test_skill_docs_reference_structured_version_sync() {
  # `.agents/skills/...` is a local-only mirror (absent in a clean public checkout).
  # Only assert skill files that exist; skip missing mirrors.
  for skill in \

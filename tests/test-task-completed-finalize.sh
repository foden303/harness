#!/bin/bash
# Verify the safety conditions for finalize on TaskCompleted
...
# Case 1: intermediate tasks should not finalize
...
# Case 2: on the last task, finalize using the session.json fallback
...
case2_output="$(run_task_completed "${CASE2_DIR}" '{"teammate_name":"impl-1","task_id":"26.0.2","task_subject":"final task"}')"
...
# Case 3: re-running the same session should not send finalize twice
case3_output="$(run_task_completed "${CASE2_DIR}" '{"teammate_name":"impl-1","task_id":"26.0.2","task_subject":"final task"}')"
...
case4_output="$(run_task_completed "${CASE4_DIR}" '{"teammate_name":"impl-1","task_id":"26.0.2","task_subject":"final task"}')"

...
# Session A: 2 worker (counts=true), 1 worker status (counts=false)
cat >"${LEDGER}" <<'EOF'
{"ts":"2026-06-03T00:00:00Z","backend":"worker","subcommand":"task","write":true,"exit_code":null,"duration_ms":null,"session_id":"sess-A","counts":true}
{"ts":"2026-06-03T00:00:01Z","backend":"worker","subcommand":"review","write":false,"exit_code":null,"duration_ms":null,"session_id":"sess-A","counts":true}
{"ts":"2026-06-03T00:00:03Z","backend":"worker","subcommand":"status","write":false,"exit_code":null,"duration_ms":null,"session_id":"sess-A","counts":false}
EOF
...
if [ -f "${TOTALS}" ]; then
  ...
  [ "$(jq -r '.totals.worker' "${TOTALS}")" = "2" ] && ok "worker total=2 (status excluded)" || ng "worker total ($(jq -r '.totals.worker' "${TOTALS}"))"
...
[ "$(jq -r '.totals.worker' "${TOTALS}")" = "2" ] && ok "idempotent: worker still 2" || ng "idempotent worker ($(jq -r '.totals.worker' "${TOTALS}"))"
...
cat >>"${LEDGER}" <<'EOF'
{"ts":"2026-06-03T01:00:00Z","backend":"worker","subcommand":"task","write":true,"exit_code":null,"duration_ms":null,"session_id":"sess-B","counts":true}
EOF
run_rollup sess-B >/dev/null 2>&1
[ "$(jq -r '.totals.worker' "${TOTALS}")" = "3" ] && ok "sess-B adds: worker=3" || ng "sess-B worker ($(jq -r '.totals.worker' "${TOTALS}"))"
...
cat >"${DLEDGER}" <<'EOF'
{"ts":"2026-06-03T02:00:00Z","backend":"worker","subcommand":"task","write":true,"exit_code":0,"duration_ms":50,"session_id":"sess-D","counts":true}
{"ts":"2026-06-03T02:00:01Z","backend":"worker","subcommand":"review","write":false,"exit_code":0,"duration_ms":40,"session_id":"sess-D","counts":true}
EOF
run_delta() {
...
run_delta
[ "$(jq -r '.totals.worker' "${DTOTALS}")" = "2" ] && ok "delta: initial worker=2" || ng "delta initial ($(jq -r '.totals.worker' "${DTOTALS}"))"
# one more counted same-session delegation appears AFTER the first rollup
cat >>"${DLEDGER}" <<'EOF'
{"ts":"2026-06-03T02:00:02Z","backend":"worker","subcommand":"task","write":true,"exit_code":0,"duration_ms":60,"session_id":"sess-D","counts":true}
EOF
# re-rollup must add only the tail (2 -> 3): NOT skipped (stay 2), NOT re-added whole (5)
run_delta
[ "$(jq -r '.totals.worker' "${DTOTALS}")" = "3" ] && ok "delta: re-rollup adds tail worker 2->3" || ng "delta re-rollup ($(jq -r '.totals.worker' "${DTOTALS}"))"
...
[ "$(jq -r '.session_counts["sess-D"].worker' "${DTOTALS}")" = "3" ] && ok "delta: session_counts snapshot updated to 3" || ng "delta snapshot ($(jq -r '.session_counts["sess-D"].worker' "${DTOTALS}"))"
# re-rollup again with no new delegations -> stays 3 (no-double-count preserved)
run_delta
[ "$(jq -r '.totals.worker' "${DTOTALS}")" = "3" ] && ok "delta: no new delegations stays 3 (no-double-count)" || ng "delta no-op ($(jq -r '.totals.worker' "${DTOTALS}"))"
...
cat >"${MLEDGER}" <<'EOF'
{"ts":"2026-06-03T03:00:00Z","backend":"worker","subcommand":"task","write":true,"exit_code":null,"duration_ms":null,"session_id":"sess-M","counts":true}
{"ts":"2026-06-03T03:00:01Z","backend":"worker","subcommand":"task","write":true,"exit_code":null,"duration_ms":null,"session_id":"sess-M","counts":true}
EOF
# pre-fix totals file: sess-M already folded worker=2 into totals, no session_counts field
cat >"${MTOTALS}" <<'EOF'
{"version":1,"totals":{"worker":2},"rolled_up_sessions":["sess-M"],"first_seen":"2026-06-03T03:00:00Z","last_seen":"2026-06-03T03:00:00Z"}
EOF
...
[ "$(jq -r '.totals.worker' "${MTOTALS}")" = "2" ] && ok "migration: already-counted session not double-counted (stays 2)" || ng "migration double-count ($(jq -r '.totals.worker' "${MTOTALS}"))"
# after migration the snapshot is seeded, so a further delegation is captured as a delta
cat >>"${MLEDGER}" <<'EOF'
{"ts":"2026-06-03T03:00:02Z","backend":"worker","subcommand":"task","write":true,"exit_code":null,"duration_ms":null,"session_id":"sess-M","counts":true}
EOF
HARNESS_ORCHESTRATION_LEDGER="${MLEDGER}" HARNESS_ORCHESTRATION_TOTALS="${MTOTALS}" \
  bash "${ROLLUP}" sess-M >/dev/null 2>&1
[ "$(jq -r '.totals.worker' "${MTOTALS}")" = "3" ] && ok "migration: post-seed tail captured (worker 2->3)" || ng "migration post-seed ($(jq -r '.totals.worker' "${MTOTALS}"))"

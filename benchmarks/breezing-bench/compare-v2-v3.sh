...
# ─────────────────────────────────────────
# v2 baseline (known measured values)
# ─────────────────────────────────────────
...
info "[1/4] Running the v3 vitest test suite..."
...
if [[ -z "$V3_TEST_OUTPUT" ]]; then
  # Fall back to normal output when the JSON reporter is unavailable
...
# Fallback: parse with a different pattern
...
# ─────────────────────────────────────────
# [2] Aggregate coverage per guardrail
# ─────────────────────────────────────────
log ""
info "[2/4] Aggregating coverage per guardrail rule..."

# Count the describe blocks in rules.test.ts (proxy metric for rule coverage)
...
if [[ -f "$RULES_FILE" ]]; then
  ...
  # fallback: count by the id: field
...
# ─────────────────────────────────────────
# [3] Compute the guardrail accuracy score
# ─────────────────────────────────────────
log ""
info "[3/4] v2 vs v3 accuracy comparison..."

# v3 score = (passed_tests / total_tests) * 100
# All unit tests passing = a metric that each rule is implemented correctly
...
log "  │ Rule definitions        │ scattered (per sh)  │ ${RULE_DEFS} rules.ts │"
log "  └─────────────────────────────────────────────────┘"
log ""
...
if [[ "$VERBOSE" == "true" ]]; then
  ...
  log "  - Difference: +44.0%pt, Fisher p=0.000005, Cohen's h=0.95 (Large)"
  log ""
...
# ─────────────────────────────────────────
log ""
info "[4/4] Pass/fail determination..."

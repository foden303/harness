...
# macOS compatibility: portable wrapper for the timeout command
...
# Function to randomize the execution order
generate_execution_order() {
  ...
  for task in "${TASKS[@]}"; do
    for iter in $(seq 1 "$ITERATIONS"); do
      if [[ "$MODE" == "both" ]]; then
        # Determine the order based on the seed
...
# Function to load the task prompt
get_task_prompt() {
  local task_num="$1"
  ...
  # Extract the prompt from YAML (simple parsing)
  grep '^prompt:' "$yaml_file" | sed 's/^prompt: *//' | sed 's/^"//' | sed 's/"$//'
}

# Function to copy the setup files
setup_work_dir() {
  local task_num="$1"
  local work_dir="$2"
  local task_dir="$TASKS_DIR/task-${task_num}"
...
Make sure all existing tests still pass."
  ...
  # Run with the Claude CLI (non-interactive, skip all permissions, no session persistence)
  ( cd "$work_dir" && \
...
7. After review cycle, clean up the team"
  ...
  # Breezing uses a Team, so increase the budget
  ( cd "$work_dir" && \

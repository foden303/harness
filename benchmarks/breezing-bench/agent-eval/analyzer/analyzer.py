...
def welch_t_test(group1: List[float], group2: List[float]) -> Tuple[float, float]:
    ...
    # Use scipy if it is available
    try:
        from scipy import stats
        p_value = stats.t.sf(abs(t_stat), df) * 2
    except ImportError:
        # Approximation: substitute the normal distribution (valid when df is large)
...
def _normal_cdf(x: float) -> float:
    """CDF of the standard normal distribution (approximation)"""
...
def bootstrap_ci(
    ...
    """Compute the CI of the effect size via hierarchical bootstrap"""
    import random

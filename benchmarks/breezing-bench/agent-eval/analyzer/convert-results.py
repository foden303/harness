#!/usr/bin/env python3
"""
Script to convert agent-eval results into the Breezing analyzer format
...
Target format (what analyzer.py expects):
  converted/{timestamp}/
...
def collect_sequential_results(
    ...
    for seq_dir in sorted(results_dir.iterdir()):
        ...
        # _seq_vanilla_task-01/<timestamp>/ → task-XX lives inside the timestamp dir

...
Deterministic grader that runs hidden tests to compute a score
"""
...
class BreezingGrader:
    ...
    def __init__(self, project_dir: Path, task_dir: Path):
        """
        Args:
            project_dir: The project directory the agent worked in
            task_dir: The task definition directory (contains hidden-tests/)
        """
    ...
    def _grade_correctness(self) -> Dict[str, Any]:
        ...
        try:
            # Install with npm ci (when a lockfile exists)
            ...
            try:
                # vitest's JSON output goes to stdout
                ...
                # On JSON parse failure, parse from the text output
            ...
            # Remove the hidden tests (cleanup)
            if test_dest.exists():
                shutil.rmtree(test_dest)

    def _parse_text_output(self, result: subprocess.CompletedProcess) -> Dict[str, Any]:
        """Parse test results from the text output"""
        import re
        output = result.stdout + result.stderr

        # Parse vitest's summary line: "Tests  5 passed | 2 failed (7)"
    ...
    def _grade_typecheck(self) -> Dict[str, Any]:
        """Secondary: result of tsc --noEmit"""
        try:
    ...
    def _count_self_tests(self) -> int:
        """Secondary: number of test files the agent authored itself"""

...
// newTestStore creates a temporary DB file for tests and returns a HarnessStore.
// It is automatically closed and removed via t.Cleanup.
...
func TestSchemaInit(t *testing.T) {
	...
	// The version should be recorded in schema_meta.
...
func TestSchemaInit_Idempotent(t *testing.T) {
	// Calling NewHarnessStore twice on the same path should be fine.
	dir := t.TempDir()
...
func TestSignal_SendAndReceive(t *testing.T) {
	...
	// Note: normally you would register a session first (an INSERT into sessions is
	// not required for the FK constraint but is required for work_states; signals hold
	// from_session_id directly, so sessions is unnecessary).
	...
	// The second call should be empty since they are already consumed.
...
func TestSignal_Broadcast(t *testing.T) {
	...
	// Any session (other than the sender) should be able to receive it.
...
func TestSignal_SenderDoesNotReceiveOwn(t *testing.T) {
	...
	// The sender itself should not receive its own signal.
...
func TestWorkState_SetAndGet(t *testing.T) {
	...
	// INSERT into the sessions table first (because of the FK constraint).
...
func TestWorkState_CleanExpired(t *testing.T) {
	...
	// call it on an empty DB and confirm 0 deletions (verifying the logic works).
...
// TestConcurrentAccess confirms that no deadlock occurs even when 3 goroutines
...
func TestNewHarnessStore_CreatesDirectory(t *testing.T) {
	dir := t.TempDir()
	// Create the DB under a nonexistent subdirectory.
...
func TestResolveStatePath(t *testing.T) {
	// Set environment variables and verify the priority order.

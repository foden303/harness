...
// Ported to Go from the TypeScript core/src/state/schema.ts.
package state

// SchemaVersion is the current schema version number.
// Increment it whenever a migration becomes necessary.
...
// It must be created before all other tables.
...
// project_root: the project root the session is associated with
...
// session_id: the session that was running the task
// severity: warning | error | critical
// message: description of the failure
// detail: detailed information such as a stack trace (nullable)
...
// The successor to work-active.json. Manages the state of work/codex/breezing modes.
// session_id: the associated session ID (PK)
// codex_mode: codex mode flag (0/1)
// bypass_rm_rf: rm -rf guard bypass flag (0/1)
// bypass_git_push: git push guard bypass flag (0/1)
// expires_at: expiry (Unix timestamp seconds, 24 hours later)
...
// A table for tracking assumptions made by agents.
// id: auto-increment PK
// session_id: the session that recorded the assumption
// task_id: related task identifier (nullable)
// assumption: the assumption content (text)
// confidence: confidence (0.0 to 1.0)
// created_at: record time (Unix timestamp seconds)
// validated_at: validation time (NULL = not validated)
...
// It is recorded by the SubagentStart/Stop hooks and displayed by harness status.
// agent_id: agent identifier issued by CC (PK)
// agent_type: worker | reviewer | advisor, etc.
...
// createIndexes is the set of indexes that improve query performance.
...
// allDDL is the array of DDL executed in order during DB initialization.
// It creates schema_meta first, then each table, and finally the indexes.
var allDDL []string

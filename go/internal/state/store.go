// Package state provides Harness v4's SQLite state management.
// Ported to Go from the TypeScript core/src/state/store.ts.
//
// Driver used: modernc.org/sqlite (pure Go, no CGO required)
// WAL mode improves concurrent reads, and a busy timeout of 5s eases lock contention.
package state
...
import (
	...
	_ "modernc.org/sqlite" // register the SQLite driver (imported for side effects only)
)
...
// SessionMode represents a session's operating mode.
...
// Signal represents a signal between agents.
type Signal struct {
...
// Assumption represents an assumption recorded by an agent.
...
// WorkStateOptions holds the optional arguments for SetWorkState.
...
// ResolveStatePath resolves the State DB path following a priority order.
// Priority:
//  1. ${CLAUDE_PLUGIN_DATA}/state.db
//  2. ${PROJECT_ROOT}/.harness/state.db
//  3. Fallback: ${PROJECT_ROOT}/.claude/state/state.db (assumed read-only, but Open is still attempted)
func ResolveStatePath(projectRoot string) string {
	// 1. The plugin data directory guaranteed to persist in CC v2.1.78+.
	if pluginData := os.Getenv("CLAUDE_PLUGIN_DATA"); pluginData != "" {
		return filepath.Join(pluginData, "state.db")
	}
	// 2. The project-local .harness directory.
	if projectRoot != "" {
		return filepath.Join(projectRoot, ".harness", "state.db")
	}
	// 3. Fallback: relative to the current directory.
	cwd, err := os.Getwd()
	if err != nil {
...
// HarnessStore is Harness's SQLite state store.
// A Go implementation that ports the TypeScript HarnessStore class 1:1.
type HarnessStore struct {
	db *sql.DB
}

// NewHarnessStore opens a SQLite DB at the given path, initializes the schema, and
// returns a HarnessStore.
// It configures WAL mode and a busy timeout of 5s to ease lock contention.
func NewHarnessStore(dbPath string) (*HarnessStore, error) {
	// Create the DB file's parent directory (if absent).
	...
	// Limit the connection count to match SQLite's file-level locking.
	// In WAL mode read concurrency is high, but writes must be serialized.
	db.SetMaxOpenConns(1)

	// PRAGMA cannot use placeholders, so execute it directly with Exec.
	...
	// Set the lock-wait timeout to 5 seconds (SPEC.md §6).
...
// initSchema executes all DDL on first launch and records the version in schema_meta.
// If a version is already recorded, it does nothing (migrations to be implemented later).
func (s *HarnessStore) initSchema() error {
	// The schema_meta table must always be created first.
	...
	case err == sql.ErrNoRows:
		// First time: execute all DDL and record the version.
	...
	default:
		// Version already recorded — migrations to be implemented later.
		_ = version
	}
...
// Corresponds to the TypeScript upsertSession.
func (s *HarnessStore) UpsertSession(session SessionState) error {
	// Convert the ISO 8601 string to Unix timestamp seconds.
...
// Corresponds to the TypeScript endSession.
...
// GetSession retrieves session information. Returns nil if it does not exist.
// Corresponds to the TypeScript getSession.
...
// SendSignal sends a signal and returns the ID of the inserted row.
// Corresponds to the TypeScript sendSignal.
...
// ReceiveSignals receives unconsumed signals and marks them consumed.
// It targets signals addressed to sessionId or broadcast (to_session_id IS NULL).
// Signals sent by the caller itself are excluded.
// Corresponds to the TypeScript receiveSignals.
func (s *HarnessStore) ReceiveSignals(sessionID string) ([]Signal, error) {
	...
	// Mark the received signals as consumed.
	// Since database/sql does not directly support variadic placeholders,
...
// RecordFailure records a task failure and returns the ID of the inserted row.
// Corresponds to the TypeScript recordFailure.
...
// GetFailures retrieves a task's failure history.
// Corresponds to the TypeScript getFailures.
...
// SetWorkState registers work/codex mode (TTL 24 hours).
// Corresponds to the TypeScript setWorkState.
...
// Corresponds to the TypeScript getWorkState.
...
// CleanExpiredWorkStates deletes expired work_states and returns the number deleted.
// Corresponds to the TypeScript cleanExpiredWorkStates.
...
// GetMeta retrieves a value from the schema_meta table. Returns "" if it does not exist.
// Corresponds to the TypeScript getMeta.
...
// SetMeta stores a value in the schema_meta table (upsert).
// Corresponds to the TypeScript setMeta.
...
// RecordAssumption saves an assumption recorded by an agent and returns the ID of the inserted row.
...
// AgentStateRecord represents one row of the agent_states table.
...
// If an existing agent_id is present, it updates state, stopped_at, and recovery_attempts.
...
// ListAgentStates returns all agent states in ascending started_at order.
// If onlyActive is true, it returns only records where stopped_at is NULL.
...
// unixToISO converts Unix timestamp seconds to an ISO 8601 UTC string.
func unixToISO(unixSec int64) string {
	return time.Unix(unixSec, 0).UTC().Format(time.RFC3339)
}

// parseISOToUnix converts an ISO 8601 string to Unix timestamp seconds.
...
// buildPlaceholders returns a string of N "?" joined by commas.

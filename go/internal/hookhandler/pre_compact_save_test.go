...
func TestPreCompactSave_SavesArtifact(t *testing.T) {
	...
	// verify handoff-artifact.json was generated
...
func TestPreCompactSave_WIPTasksExtracted(t *testing.T) {
	...
	// verify the response message contains the WIP task count
...
func TestPreCompactSave_SavesLegacySnapshot(t *testing.T) {
	...
	// verify precompact-snapshot.json was generated
...
func TestPreCompactSave_SecuritySymlinkCheck(t *testing.T) {
	...
	// make the artifact path a symlink
...
// TestBuildOpenRisks_SessionMetricsQualityRisk verifies that session-metrics.json's
// failed_checks are converted into a quality risk in open_risks (test for fix #3).
func TestBuildOpenRisks_SessionMetricsQualityRisk(t *testing.T) {
	...
	// case where session-metrics has 2 failed_checks
	...
	// verify a quality risk is present
	found := false
	for _, r := range risks {
...
// TestBuildOpenRisks_SessionMetrics_failedChecks_Field verifies that session-metrics'
// failedChecks (camelCase) field is also converted into a quality risk.
...
// TestBuildOpenRisks_NoMetrics verifies that when metrics is nil, no quality risk
// is added.
...
// TestBuildOpenRisks_EmptyMetricsFailedChecks verifies that when failed_checks is an empty array
// no quality risk is added.
...
// TestPreCompactSave_SessionMetricsOpenRisks verifies end-to-end that handoff-artifact.json's
// open_risks contains session-metrics.json failures as a quality risk
...
func TestPreCompactSave_SessionMetricsOpenRisks(t *testing.T) {
	...
	// getSessionMetrics reads repoRoot/.claude/state/session-metrics.json
	...
	// verify open_risks contains a quality risk
	found := false
...
// TestBuildOpenRisks_ReviewStatusPassed verifies that when review_status="passed"
// no risk is added (symmetry with the bash/JS versions).
...
// TestBuildOpenRisks_ReviewStatusFailed verifies that when review_status="failed"
// a high-severity review risk is added.
...
// TestBuildOpenRisks_ReviewStatusPending verifies that when review_status="pending"
// a medium-severity review risk is added.
...
// a single map object, it is wrapped in an array and processed (symmetry with the JS version).
...
// TestPreCompactSave_CustomPlansDirectory verifies that when plansDirectory is set
// the custom-directory Plans.md is read via resolvePlansPath (P1 fix).
func TestPreCompactSave_CustomPlansDirectory(t *testing.T) {
	...
	// leave PlansFile empty and defer to resolvePlansPath (PlansFile="" -> refers to config)
	h := &PreCompactSave{
		RepoRoot: dir,
		StateDir: stateDir,
		// intentionally do not set PlansFile
	}
	...
	// WIP task should be detected (evidence that the custom-directory Plans.md was read)

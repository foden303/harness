...
func TestHandleTrackChanges_WindowsPathNormalization(t *testing.T) {
	...
	// A path containing Windows backslashes
	...
	// Backslashes should be converted to slashes
...
func TestHandleTrackChanges_CWDRelativePath(t *testing.T) {
	...
	// Convert an absolute path with CWD into a relative path
...
func TestHandleTrackChanges_DedupWithin2Hours(t *testing.T) {
	...
	// The second call should be skipped by dedup
	out.Reset()
	if err := HandleTrackChanges(strings.NewReader(input), &out); err != nil {
	...
	// The second call is skipped, so only 1 line
	if len(lines) != 1 {

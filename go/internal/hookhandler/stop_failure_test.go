...
func TestStopFailureHandler_EmptyInput(t *testing.T) {
	...
	// Output nothing for empty input
	if out.Len() != 0 {
...
func TestStopFailureHandler_RateLimit429_SystemMessage(t *testing.T) {
	...
	// For 429, a systemMessage is output
...
func TestStopFailureHandler_NonRateLimit_NoSystemMessage(t *testing.T) {
	...
	// For anything other than 429, no systemMessage is output
	output := strings.TrimSpace(out.String())
	if output != "" {
...
func TestStopFailureHandler_StringError(t *testing.T) {
	// Test for when the error field is a string
	dir := t.TempDir()
	t.Setenv("CLAUDE_PLUGIN_DATA", "") // Unset CLAUDE_PLUGIN_DATA to use the default path
	h := &StopFailureHandler{ProjectRoot: dir}
...
func TestStopFailureHandler_Idempotent(t *testing.T) {
	// Verify multiple calls append multiple lines to the JSONL
	dir := t.TempDir()
	t.Setenv("CLAUDE_PLUGIN_DATA", "") // Unset CLAUDE_PLUGIN_DATA to use the default path
	h := &StopFailureHandler{ProjectRoot: dir}
...
// --- Finding 3: CLAUDE_PLUGIN_DATA support test ---
...
func TestResolveStopFailureStateDir_WithPluginData(t *testing.T) {
	...
	// Expected hash: first 12 chars of the CWD's SHA-256
...
func TestStopFailureHandler_CLAUDE_PLUGIN_DATA_UsesHashedPath(t *testing.T) {
	// Verify that when CLAUDE_PLUGIN_DATA is set, logs are written to the hashed path
	dir := t.TempDir()
	pluginData := t.TempDir()
	t.Setenv("CLAUDE_PLUGIN_DATA", pluginData)
	...
	// Verify the log was written to the hashed path
...
// --- Findings 1+2: fireWebhook synchronization and payload/header match test ---

func TestFireWebhook_SynchronousWithCorrectHeaderAndBody(t *testing.T) {
	// Spin up a test HTTP server and verify the request contents
	...
	// Since it runs synchronously, it should have reached the server after return
...
func TestFireWebhook_NoURL_NoOp(t *testing.T) {
	...
	h := &taskCompletedHandler{}
	// Verify it does not panic and does not time out

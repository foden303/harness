...
func TestHandleTDDOrderCheck_SkipTDDMarker(t *testing.T) {
	...
	// skip when the [skip:tdd] marker is present
...
func TestHandleTDDOrderCheck_WarningEmitted(t *testing.T) {
	...
	// WIP task present, tests not edited, no [skip:tdd] -> emit a warning
...
func TestHandleTDDOrderCheck_TestAlreadyEdited(t *testing.T) {
	...
	// WIP task present, test file recorded in session-changes.json
	if err := os.WriteFile("Plans.md", []byte("| Task | implement | DoD | - | cc:WIP |\n"), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestHandleTDDOrderCheck_TestAlreadyInChangedFiles(t *testing.T) {
	...
	// WIP task present, test file recorded in changed-files.jsonl
	if err := os.WriteFile("Plans.md", []byte("| Task | implement | DoD | - | cc:WIP |\n"), 0o644); err != nil {
		t.Fatal(err)
	}
	if err := os.MkdirAll(".claude/state", 0o755); err != nil {
...
// TestHasActiveWIPTask_CustomPlansDirectory verifies that when plansDirectory is set
// WIP tasks are detected from the Plans.md in the custom directory (P3 fix).
func TestHasActiveWIPTask_CustomPlansDirectory(t *testing.T) {
	...
	// place a Plans.md containing cc:WIP in the docs/ directory
...
// TestHasActiveWIPTask_CustomPlansDirectory_NoWIP verifies that when the custom-directory
// Plans.md has no cc:WIP, false is returned.
...
// TestHasActiveWIPTask_NoPlansFile verifies false is returned when Plans.md does not exist.

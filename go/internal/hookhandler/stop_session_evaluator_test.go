...
// assertStopOK is a helper that verifies the ok field of the output JSON.
...
func TestStopSessionEvaluator_NoStateFile(t *testing.T) {
	// ok: true when session.json does not exist
	dir := t.TempDir()
...
func TestStopSessionEvaluator_RecordsLastMessage(t *testing.T) {
	...
	// Check that last_message_length and last_message_hash were written to session.json
	...
	// Check that the plaintext message is not saved
	content := string(data)
...
`
	...
	// Does not block (ok: true)
	if !resp.OK {
		t.Errorf("ok = false, want true (WIP should not block stop)")
	}
	// systemMessage should contain the warning

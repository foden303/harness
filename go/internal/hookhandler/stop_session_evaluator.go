...
// stopSessionInput is the stdin JSON payload of the Stop hook.
...
// stopSessionResponse is the response of the Stop hook.
...
//
// It evaluates the session state on the Stop event.
//   - Records last_assistant_message as its length and hash (first 16 chars of SHA-256) in session.json
//   - Warns via systemMessage if WIP tasks remain (does not block)
//   - Stopping is always allowed (ok: true)
...
func (h *StopSessionEvaluatorHandler) Handle(in io.Reader, out io.Writer) error {
	...
	// Default to ok if session.json does not exist
	...
	// If already in stopped state, immediately ok
	if state, ok := sessionMap["state"].(string); ok && state == "stopped" {
		return writeJSON(out, stopSessionResponse{OK: true})
	}

	// WIP task check: find Plans.md and count cc:WIP
...
// The plaintext content is not stored (privacy protection).
func (h *StopSessionEvaluatorHandler) recordLastMessage(stateFile, msg string) {
	// Skip if the file does not exist (same behavior as the bash version)
	sessionData, err := os.ReadFile(stateFile)
	if err != nil {
		return
	}
...
// countWIPTasks finds Plans.md under projectRoot and returns the number of cc:WIP markers.

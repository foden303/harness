...
// TestPostToolUseLogToolName_AlwaysContinue verifies it always returns continue=true.
...
// TestPostToolUseLogToolName_EmptyInput verifies it returns continue=true even for empty input.
...
// TestPostToolUseLogToolName_Phase0Log verifies recording into tool-events.jsonl when CC_HARNESS_PHASE0_LOG=1.
func TestPostToolUseLogToolName_Phase0Log(t *testing.T) {
	...
	// tool-events.jsonl should have been created
...
// TestPostToolUseLogToolName_Phase0LogDisabled verifies no recording when CC_HARNESS_PHASE0_LOG is unset.
func TestPostToolUseLogToolName_Phase0LogDisabled(t *testing.T) {
	...
	// tool-events.jsonl should not have been created
	logFile := filepath.Join(stateDir, toolEventsFile)
...
// TestPostToolUseLogToolName_LSPTracking verifies LSP tool usage is recorded into tooling-policy.json.
func TestPostToolUseLogToolName_LSPTracking(t *testing.T) {
	...
	// initialize session.json and tooling-policy.json
...
// TestPostToolUseLogToolName_NonLSPToolNoPolicy verifies a non-LSP tool does not change tooling-policy.json.
func TestPostToolUseLogToolName_NonLSPToolNoPolicy(t *testing.T) {
	...
	// tooling-policy.json should be unchanged
...
// TestPostToolUseLogToolName_SessionEventLog verifies important tools are recorded in session events.
...
// TestPostToolUseLogToolName_SkillTracking verifies Skill tool usage is recorded into session-skills-used.json.
...
// TestPostToolUseLogToolName_SkillTrackingMultiple verifies multiple Skill usages accumulate.
...
// TestIsImportantTool verifies the important-tool decision logic.
...
// TestNeedsRotation verifies the rotation-decision logic.
func TestNeedsRotation(t *testing.T) {
	dir := t.TempDir()
	path := filepath.Join(dir, "test.jsonl")

	// no file -> no rotation needed
	if needsRotation(path, 100, 10) {
		t.Error("expected false for non-existent file")
	}

	// small file -> no rotation needed
...
// TestBuildEventData verifies the event-data build logic.

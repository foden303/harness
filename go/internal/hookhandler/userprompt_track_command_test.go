...
func TestTrackCommandHandler_NonSlashPrompt(t *testing.T) {
	...
	var out bytes.Buffer
	err := h.Handle(strings.NewReader(`{"prompt":"a normal message"}`), &out)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestTrackCommandHandler_SkillRequiredCommand_Work(t *testing.T) {
	...
	// Check that the pending file was created
...
func TestTrackCommandHandler_PluginPrefixCommand(t *testing.T) {
	...
	var out bytes.Buffer
	err := h.Handle(strings.NewReader(`{"prompt":"/harness:core:work run the task"}`), &out)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	// Check that the prefix is stripped and it is recognized as work
	pendingFile := filepath.Join(dir, ".claude", "state", "pending-skills", "work.pending")
	if _, err := os.ReadFile(pendingFile); err != nil {
...
func TestTrackCommandHandler_MultilinePrompt(t *testing.T) {
	...
	// Verify the behavior of checking only the first line
	prompt := "/work\nnext line content\nand the next line"
	...
	// /work should be recognized as a command
	pendingFile := filepath.Join(dir, ".claude", "state", "pending-skills", "work.pending")
	if _, err := os.ReadFile(pendingFile); err != nil {
...
func TestTrackCommandHandler_LongPromptTruncated(t *testing.T) {
	...
	// A prompt exceeding 200 characters
	...
	// preview is at most 200 characters (in runes)

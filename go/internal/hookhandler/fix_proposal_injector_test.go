...
// makeFixProposalFile creates a pending-fix-proposals.jsonl file for tests.
...
// sampleProposal returns a fixProposal for tests.
func sampleProposal() fixProposal {
	return fixProposal{
		SourceTaskID:      "26",
		FixTaskID:         "26.fix",
		ProposalSubject:   "fix: test repair task",
		DoD:               "tests pass",
		Depends:           "26",
		FailureCategory:   "assertion_error",
		RecommendedAction: "fix the assertion",
		Status:            "pending",
	}
}

// TestFixProposalInjector_EmptyInput verifies no output is produced for empty input.
...
// TestFixProposalInjector_NoPendingFile verifies no output when there is no file.
...
// TestFixProposalInjector_Reminder verifies pending proposals are shown as a reminder.
func TestFixProposalInjector_Reminder(t *testing.T) {
	...
	var out bytes.Buffer
	err := h.Handle(strings.NewReader(`{"prompt":"do the next task"}`), &out)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
// TestFixProposalInjector_Approve verifies the "approve fix" command applies to Plans.md.
func TestFixProposalInjector_Approve(t *testing.T) {
	dir := t.TempDir()
	makeFixProposalFile(t, dir, []fixProposal{sampleProposal()})

	// Create Plans.md
	plansContent := "| Task | Content | DoD | Depends | Status |\n|------|------|-----|---------|--------|\n| 26 | original task | done | - | cc:done |\n"
	...
	if !strings.Contains(resp.SystemMessage, "Applied fix proposal") {
		t.Errorf("expected success message, got: %s", resp.SystemMessage)
	}

	// The fix task must be added to Plans.md
	...
	// The proposal must be removed from the proposals file
...
// TestFixProposalInjector_Reject verifies the "reject fix" command removes the proposal.
func TestFixProposalInjector_Reject(t *testing.T) {
	...
	if !strings.Contains(resp.SystemMessage, "Rejected fix proposal") {
...
// TestFixProposalInjector_YesApprove verifies "yes" approves a single proposal.
...
// TestFixProposalInjector_MultipleProposals_RequiresID verifies an ID is required when multiple proposals exist.
func TestFixProposalInjector_MultipleProposals_RequiresID(t *testing.T) {
	...
	// "yes" cannot be used when multiple proposals exist
...
// TestFixProposalInjector_NotFoundID verifies the error when a nonexistent ID is specified.
func TestFixProposalInjector_NotFoundID(t *testing.T) {
	...
	if !strings.Contains(resp.SystemMessage, "not found") {
		t.Errorf("expected not-found message, got: %s", resp.SystemMessage)
	}
}

// TestParseFixProposalAction verifies the action parsing logic.
...
// TestApplyFixProposalToPlans verifies the insertion logic into Plans.md.
func TestApplyFixProposalToPlans(t *testing.T) {
	...
	// 26.fix must be inserted right after 26
	lines := strings.Split(text, "\n")
	found26 := -1
	found26fix := -1
	for i, line := range lines {
		// Match "| 26 |" exactly (excluding "| 26.fix |")
	...
	// The second apply is already_present
...
// TestConsumeFixProposal verifies consume removes the specified proposal.
...
// TestFixProposalInjector_CustomPlansDirectory verifies that when the plansDirectory
// setting is present, the change is correctly applied to the custom directory's Plans.md.
func TestFixProposalInjector_CustomPlansDirectory(t *testing.T) {
	dir := t.TempDir()
	makeFixProposalFile(t, dir, []fixProposal{sampleProposal()})

	// Create the config file (plansDirectory: workspace)
	...
	// Specify only ProjectRoot without an explicit PlansPath (let it auto-resolve)
	h := &FixProposalInjectorHandler{ProjectRoot: dir}
	...
	// The fix task must be applied to workspace/Plans.md
	if !strings.Contains(resp.SystemMessage, "Applied fix proposal") {

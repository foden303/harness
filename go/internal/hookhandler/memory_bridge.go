...
// MemoryBridgeClient performs HTTP integration with the harness-mem daemon.
// In environments where harness-mem is not running it operates fail-silent,
// and only the existing JSONL log is recorded.
type MemoryBridgeClient struct {
	// HTTPClient is DI for testing. If nil, a default client with a 2s timeout is used.
	HTTPClient *http.Client
	// BaseURL is an override for testing. If empty, it is built from environment variables.
	BaseURL string
}
...
var bridgeToEventType = map[string]string{
	...
	// "stop" uses /v1/sessions/finalize, so it is not included in this map.
}
...
func (c *MemoryBridgeClient) Handle(in io.Reader, out io.Writer) error {
	...
	if !validTargets[target] {
		...
		fmt.Fprintf(os.Stderr, "[harness] unknown memory bridge target: %s\n", target)
	...
	if logErr := logMemoryBridgeEvent(target, input.SessionID, input.CWD); logErr != nil {
		...
		fmt.Fprintf(os.Stderr, "[harness] memory-bridge log error: %v\n", logErr)
	}
	...
	if err := validateBridgeInput(input); err != nil {
		fmt.Fprintf(os.Stderr, "[harness] memory-bridge validation failed: %v\n", err)
	} else {
...
// It prevents invalid events from being promoted to long-term memory (K-1.2: audit memory before storing).
...
// postToHarnessMem HTTP POSTs the event to the harness-mem daemon.
// Connection failures, timeouts, and error responses are all ignored with only a stderr log.
// In environments where harness-mem is not running, it returns immediately on connection refused.
func (c *MemoryBridgeClient) postToHarnessMem(target string, input memoryBridgeInput) {
	...
	if err != nil {
		...
		fmt.Fprintf(os.Stderr, "[harness] harness-mem POST failed (target=%s): %v\n", target, err)
		return
	}
	defer resp.Body.Close()

	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		fmt.Fprintf(os.Stderr, "[harness] harness-mem HTTP %d for %s\n", resp.StatusCode, target)
	}
}

...
// TestHandleTaskCompleted_TerminalSequenceEmit verifies that when
// HARNESS_TERMINAL_NOTIFY=osc9 is set, a terminalSequence field is added
// to the TaskCompleted response (CC 2.1.141+ Phase 69).
...
// HARNESS_TERMINAL_NOTIFY is unset, no terminalSequence is added (preserving existing behavior).
...
func TestRotateJSONLFile(t *testing.T) {
	...
	// The last line is the original entry number 549
...
func TestCountCompleted_NoDuplication(t *testing.T) {
	...
	// The same ID is counted only once
...
func TestUpsertFixProposal_CreateAndUpdate(t *testing.T) {
	...
	// The file contains only the most recent entry
...
func TestWriteFinalizeMarker_StateDirSymlink(t *testing.T) {
	...
	// stateDir is a symlink → writeFinalizeMarker writes nothing
	h.writeFinalizeMarker("sess1", "my-project", "2026-01-01T00:00:00Z")

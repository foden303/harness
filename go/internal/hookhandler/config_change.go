...
// Payload of the ConfigChange event.
...
//
// Invoked on the ConfigChange event; records to
// .claude/state/breezing-timeline.jsonl only when Breezing is active.
...
func HandleConfigChange(in io.Reader, out io.Writer) error {
	...
	// Resolve PROJECT_ROOT (prefer env var, else cwd)
	...
	// Normalize file_path to a repo-relative path (hides usernames, etc.).

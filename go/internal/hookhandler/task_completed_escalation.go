...
// testResultFile is the schema of .claude/state/test-result.json.
...
// When testOK=false it also returns failCount.
func (h *taskCompletedHandler) checkTestResultAndEscalate(taskID, taskSubject, teammateName, ts string) (testOK bool, failCount int) {
	...
	// If there is no result file, treat it as success (project with no tests)
	...
	// Record the failure in the timeline
...
// The return value is the new count.
func (h *taskCompletedHandler) updateFailureCount(taskID, action, ts string) int {
	...
	// Write back to the file (reject symlinks)
	if info, err := os.Lstat(gatePath); err == nil && info.Mode()&os.ModeSymlink != 0 {
...
// classifyFailure classifies the failure category and recommended action from the test output.
func classifyFailure(output string) (category, action string) {
	...
	case containsAny(lower, "type.*error", "typeerror", "is not assignable", "property.*does not exist"):
		return "type_error", "Fix the type error. Check mismatches between type definitions and implementation."
	case containsAny(lower, "assertion", "assertionerror", "expect.*received", "tobe", "toequal", "fail", "failed"):
		return "assertion_error", "A test assertion failed. Check the difference between expected and actual values."
	case containsAny(lower, "timeout", "etimedout", "timed out"):
		return "timeout", "A timeout occurred. Check asynchronous processing and network dependencies."
	...
	default:
		return "runtime_error", "A runtime error occurred. Inspect the test output in detail."
	}
}

// containsAny checks whether the text contains any of the candidates.
...
func (h *taskCompletedHandler) emitEscalationResponse(out io.Writer, taskID, taskSubject string, failCount int) error {
	...
	fmt.Fprint(os.Stderr, "[ESCALATION] Detected 3 consecutive failures - stopping automatic fix loop\n")
	fmt.Fprintf(os.Stderr, "==========================================\n")
	...
	fmt.Fprintf(os.Stderr, "------------------------------------------\n")
	fmt.Fprintf(os.Stderr, "  [Cause classification]\n  Category      : %s\n\n", category)
	fmt.Fprintf(os.Stderr, "  [Recommended action]\n  %s\n\n", action)
	if lastCmd != "" {
		fmt.Fprintf(os.Stderr, "  [Last command]\n  %s\n\n", lastCmd)
	}
	if lastOutput != "" {
		fmt.Fprint(os.Stderr, "  [Test output (max 20 lines)]\n")
	...
	// Generate and save the Fix Proposal
	fixTaskID := buildFixTaskID(taskID)
	proposalSubject := sanitizeInlineText("fix: " + taskSubject + " - " + category)
	dod := sanitizeInlineText("Resolve failure category (" + category + ") and make the latest test/CI pass")
...
// If an entry with the same source_task_id exists, it is replaced.
func (h *taskCompletedHandler) upsertFixProposal(proposal fixProposal) bool {
	...
	// Read existing entries (excluding the same source_task_id)

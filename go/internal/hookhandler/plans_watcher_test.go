...
func TestHandlePlansWatcher_NewTaskDetected(t *testing.T) {
	...
	// create Plans.md content containing pm:pending
	plansContent := "| Task 1 | impl A | DoD | - | pm:pending |\n"
...
func TestHandlePlansWatcher_CompletedTaskDetected(t *testing.T) {
	...
	// create Plans.md content containing cc:done
	plansContent := "| Task 1 | impl A | DoD | - | cc:done |\n"
	if err := os.WriteFile("Plans.md", []byte(plansContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestHandlePlansWatcher_NoChange(t *testing.T) {
	...
	// no change (still one cc:TODO)
	plansContent := "| Task 1 | impl A | DoD | - | cc:TODO |\n"
	if err := os.WriteFile("Plans.md", []byte(plansContent), 0o644); err != nil {
		t.Fatal(err)
	}
...
func TestHandlePlansWatcher_StatusSummary(t *testing.T) {
	...
	// summary should contain the status counts
...
func TestIsPlansFile(t *testing.T) {
	// isPlansFile matches strictly via filepath.Clean only.
	// comparison accounting for projectRoot is done by isPlansFileWithRoot.
	cases := []struct {
		changed  string
		plans    string
		expected bool
		...
		// mismatch if full paths differ (same-named file in another dir is false)
...
func TestIsPlansFileWithRoot(t *testing.T) {
	...
	cases := []struct {
		...
		// relative path -> resolved via projectRoot and matches
		{"Plans.md", "/home/user/project/Plans.md", true, "relative path match"},
		// matches by absolute path
		{"/home/user/project/Plans.md", "/home/user/project/Plans.md", true, "absolute path match"},
		// relative path in another dir (Plans.md but plansFile is directly under projectRoot)
		{"docs/Plans.md", "/home/user/project/Plans.md", false, "subdirectory mismatch"},
		// case where a relative path in another dir matches plansFile
...
func TestHandlePlansWatcher_CustomPlansDirectory(t *testing.T) {
	...
	// create the config file (plansDirectory: docs)
	...
	// send an event that changed docs/Plans.md
	...
	// new task should be detected (the custom-path Plans.md is recognized)
	if !strings.Contains(result.HookSpecificOutput.AdditionalContext, "New task") {
...
// TestIsPlansFile_CustomPath verifies that for a custom-path Plans.md
// isPlansFileWithRoot works correctly (P2 fix: do not mis-match a same-named file in another dir).
func TestIsPlansFile_CustomPath(t *testing.T) {
	...
	cases := []struct {
		...
		// exact match (relative path resolved by projectRoot)
		{"Plans.md", "/project/Plans.md", true, "exact match via projectRoot"},
		// the custom-directory Plans.md matches plansFile
		{"docs/Plans.md", "/project/docs/Plans.md", true, "custom subdir match"},
		// same-named file in another dir must not mis-match (core of the fix)
		...
		// filename resembles Plans.md but is a different file
		{"Plans.md.bak", "/project/Plans.md", false, "backup file not matched"},
		// Plans.md of a different project by absolute path
...
// input.CWD is used as projectRoot instead of resolveProjectRoot().
// verifies Plans.md is detected correctly when the hook process CWD differs from input.CWD.
func TestHandlePlansWatcher_CWDFromInput(t *testing.T) {
	...
	// the hook process is in hookCWD (not the project root)
	if err := os.Chdir(hookCWD); err != nil {
		t.Fatal(err)
	}
	defer os.Chdir(origDir)
	...
	// include a cwd field in input (pointing to projectDir)
	inputJSON := `{"tool_name":"Edit","cwd":"` + projectDir + `","tool_input":{"file_path":"Plans.md"}}`
	var out bytes.Buffer
	if err := HandlePlansWatcher(strings.NewReader(inputJSON), &out); err != nil {
		t.Fatalf("unexpected error: %v", err)
	...
	// verify it is processed without error (Plans.md is found and state aggregation proceeds)
	var result postToolOutput
	if jsonErr := json.Unmarshal(out.Bytes(), &result); jsonErr != nil {
		t.Fatalf("invalid JSON output: %v, raw: %s", jsonErr, out.String())
	}
	// should be processed normally (if hookCWD has no Plans.md it would be emptyPostToolOutput, but
	// using input.CWD finds projectDir's Plans.md)
	if out.Len() == 0 {
		t.Error("expected non-empty output when input.CWD points to project with Plans.md")
	}
}

// TestAcquirePlansLock_BasicLock verifies that lock acquire and release work correctly.
func TestAcquirePlansLock_BasicLock(t *testing.T) {
	tmpDir := t.TempDir()
	lockPath := filepath.Join(tmpDir, "locks", "plans.flock")

	// first time: lock acquired successfully
...
// fails after 3 retries, HandlePlansWatcher emits a fail-closed signal.
// mock out exitFailClosed to avoid os.Exit(1) while
// verifying the call occurred (failure signal).
func TestHandlePlansWatcher_LockExhaustionFailsClosed(t *testing.T) {
	...
	// create the lock directory and a lock file with 000 permission to make open fail
	...
	// fail-closed: exitFailClosed was called (a not-treated-as-success signal)
	if !failClosedCalled {
	...
	// when os.Exit is mocked out, the fallback empty response is returned
	if result.HookSpecificOutput.AdditionalContext != "" {
...
// invoke the hook with different CWD values and verify each path is based on input.CWD.
// this prevents the problem where a lock in CWD A does not protect state in CWD B (a race).
func TestHandlePlansWatcher_LockAndStateUseSameCWD(t *testing.T) {
	// project A directory
	projectA := t.TempDir()
	// project B directory (a different CWD)
	projectB := t.TempDir()
	...
	// set the process cwd to projectB (input.CWD points to projectA)
	if err := os.Chdir(projectB); err != nil {
		t.Fatal(err)
	}
	defer os.Chdir(origDir)

	// create Plans.md and the state directory in projectA
	plansContent := "| Task 1 | A | DoD | - | pm:pending |\n"
	...
	// invoke the hook with input.CWD = projectA
	...
	// the lock file should be created under projectA (not projectB)
	...
	// the state file should be created under projectA (not projectB)
...
// TestAcquirePlansLock_FailClosed verifies that when lock acquisition fails, HandlePlansWatcher
// becomes fail-closed (calls exitFailClosed).
// within the same process Flock is re-entrant, so lock contention occurs between
// two processes on the same file. this test makes the lock file read-only to
// make open itself fail, covering the fail-closed path.
// NOTE: TestHandlePlansWatcher_LockExhaustionFailsClosed does an equivalent check, so
// this test focuses on verifying acquirePlansLock alone.
func TestAcquirePlansLock_FailClosed(t *testing.T) {
	...
	// fail-closed fallback: on lock acquisition failure, empty AdditionalContext (no notification)
	if result.HookSpecificOutput.AdditionalContext != "" {
...
// TestReleasePlansLock_Nil verifies releasePlansLock does not panic on nil.

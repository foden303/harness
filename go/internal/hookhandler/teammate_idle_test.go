...
// assertTeammateIdleApprove is a helper that verifies an approve response.
...
// assertTeammateIdleStop is a helper that verifies a stop response.
...
func TestHandleTeammateIdle_WritesTimeline(t *testing.T) {
	...
	// Verify the timeline file was created
...
func TestHandleTeammateIdle_DedupAfterWindow(t *testing.T) {
	...
	// Manually write an old timeline entry (6 seconds ago)
	...
	// Call with the same teammate -> not skipped since more than 5 seconds elapsed
...
func TestHandleTeammateIdle_AgentNameFallback(t *testing.T) {
	...
	// agent_name field (fallback for teammate_name)

...
// It cleans up worktree-specific temporary files when a Breezing subagent exits.
//
...
// worktreeRemoveResponse is the response of the WorktreeRemove hook.
type worktreeRemoveResponse struct {
	Decision string `json:"decision"`
	Reason   string `json:"reason"`
}

// Handle reads the WorktreeRemove payload from stdin, removes the worktree-specific
...
func (h *WorktreeRemoveHandler) Handle(r io.Reader, w io.Writer) error {
	...
	// Remove Codex prompt temporary files
	removeTmpGlob("/tmp/codex-prompt-*.md")

	// Remove Harness Codex logs
...
// removeTmpGlob removes files under /tmp matching the glob pattern.

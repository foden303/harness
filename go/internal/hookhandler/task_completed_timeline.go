...
// breezingActiveJSON is the schema of breezing-active.json (only the required fields).
...
// appendTimeline appends an entry to the timeline file and rotates it when it exceeds 500 lines.
...
// updateBreezingSignals manages the Breezing session's signals (50%/60%/complete).
...
func (h *taskCompletedHandler) updateBreezingSignals(taskID, ts string) (totalTasks, completedCount int) {
	...
	// Get the batch ID list and the total task count
	var batchIDs []string
	...
	// If there is no batch info, use the plans_md_mapping keys
	...
	// Count completions from the timeline
	...
	if totalTasks > 0 {
		...
		// 60% completion signal (round-up calculation)
...
// countCompleted counts the number of completions for the given batch IDs from the timeline.
func (h *taskCompletedHandler) countCompleted(batchIDs []string) int {
	...
	// Convert the batch IDs to a set
	idSet := make(map[string]bool, len(batchIDs))
	for _, id := range batchIDs {
		idSet[strings.TrimSpace(id)] = true
	}

	// Scan the timeline and check for completion of each batch ID (max 1 count per ID)
...
// signalExists checks whether the given signal has already been recorded in the session scope.

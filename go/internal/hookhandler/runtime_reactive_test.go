...
// assertReactiveApprove is a helper that verifies an approve response.
...
// assertReactiveHookOutput is a helper that verifies a hookSpecificOutput response.
...
func TestHandleRuntimeReactive_FileChanged_PlansmdWithPath(t *testing.T) {
	...
	// Case of an absolute path that includes the project root
...
func TestHandleRuntimeReactive_FileChanged_UnrelatedFile(t *testing.T) {
	...
	// For an unrelated file, return approve
...
func TestHandleRuntimeReactive_TaskCreated(t *testing.T) {
	...
	// TaskCreated logs only -> returns approve
	assertReactiveApprove(t, out.String(), "Reactive hook tracked")
}
...
func TestHandleRuntimeReactive_AlternativeFieldNames(t *testing.T) {
	...
	// Use the alternative fields event_name and path

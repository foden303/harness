...
func TestHandlePostToolUseQualityPack_EnabledWarnMode(t *testing.T) {
	...
	// Create config file (enabled, warn mode)
	config := `quality_pack:
  enabled: true
  mode: warn
  prettier: true
  ...
  console_log: true
`
	if err := os.WriteFile(".harness.config.yaml", []byte(config), 0o644); err != nil {
		t.Fatal(err)
	}

	// Create TS file (containing console.log)
...
`
	...
	// no console.log + prettier/tsc disabled → no feedback → no output
	if out.Len() != 0 {
...
`
	...
	// Verify CWD prefix removal (absolute path → relative path conversion)
...
func TestIsJSTSFile(t *testing.T) {
	cases := []struct {
		...
		{"src/foo.py", false},
		{"src/foo.md", false},
		{"src/FOO.TS", true}, // case insensitive
	}

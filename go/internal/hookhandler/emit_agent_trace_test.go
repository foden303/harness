...
func TestEmitAgentTrace_NoSupportedTool(t *testing.T) {
	...
	// Verify the trace file is not created
...
// TestEmitAgentTrace_WriteTool_AlwaysCreate verifies that the Write tool returns
// action=create even for existing files.
// At PostToolUse time the file has already been written, so os.Stat cannot
// distinguish new vs existing. Since the decision is tool_name based, Write is
// always treated as "create".
func TestEmitAgentTrace_WriteTool_AlwaysCreate(t *testing.T) {
	dir := t.TempDir()

	// Create the existing file beforehand
	...
	// Write is tool_name based, so even an existing file yields "create"
	if rec.Files[0].Action != "create" {
...
func TestEmitAgentTrace_Rotation(t *testing.T) {
	...
	// Create a small file and set the rotation threshold
	...
	e := &EmitAgentTrace{
		RepoRoot:    dir,
		StateDir:    stateDir,
		MaxFileSize: 10, // rotate at 10 bytes
...
func TestEmitAgentTrace_SecuritySymlinkCheck(t *testing.T) {
	...
	// Verify the symlink is avoided without an error
	if err := e.Handle(strings.NewReader(""), nil); err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestEmitAgentTrace_PathWithinRepo(t *testing.T) {
	...
	// Create an existing file and test with an absolute path
...
// TestEmitAgentTrace_OtelParallel verifies that the OTel POST runs in parallel in
// a goroutine and Handle() returns without blocking.
// It uses a mock HTTP server to also verify the request reliably arrives.
func TestEmitAgentTrace_OtelParallel(t *testing.T) {
	requestReceived := make(chan struct{}, 1)

	// mock HTTP server: notify the channel when a request is received
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
		select {
	...
	// By the time Handle() returns, wg.Wait() ensures the POST has completed.
	select {
	case <-requestReceived:
		// OK: the OTel server received the request
	case <-time.After(3 * time.Second):
		t.Error("OTel server did not receive request within timeout")
	}

	// goroutine + wg.Wait() keeps blocking to a minimum
	// (should complete within the 5s HTTP timeout)
	if elapsed > 10*time.Second {
		t.Errorf("Handle() took too long: %v (expected < 10s)", elapsed)
	}
}

// TestEmitAgentTrace_ChmodExistingFile verifies an existing file's permissions are corrected to 0600.
func TestEmitAgentTrace_ChmodExistingFile(t *testing.T) {
	...
	// Create an existing agent-trace.jsonl with 0644 (must be corrected to 0600)
	tracePath := filepath.Join(stateDir, "agent-trace.jsonl")
	...
	// Verify the file permission is 0600
	info, err := os.Stat(tracePath)
	if err != nil {
		t.Fatalf("trace file not found: %v", err)
	}

	// Extract only the permission bits from the file mode
	perm := info.Mode().Perm()
	if perm != 0600 {
...
// TestEmitAgentTrace_MultiEditTool_ModifyAction verifies the MultiEdit tool
// returns action=modify.

...
// TestUserPromptInjectPolicy_EmptyInput verifies it exits normally even with empty input.
...
// TestUserPromptInjectPolicy_NoStateDir verifies it returns empty output when there is no state directory.
func TestUserPromptInjectPolicy_NoStateDir(t *testing.T) {
	dir := t.TempDir()
	h := &UserPromptInjectPolicyHandler{ProjectRoot: dir}

	input := `{"prompt": "do some work"}`
	var out bytes.Buffer
...
// TestUserPromptInjectPolicy_ResumeContextInjected verifies the memory resume context is injected.
func TestUserPromptInjectPolicy_ResumeContextInjected(t *testing.T) {
	...
	var out bytes.Buffer
	err := h.Handle(strings.NewReader(`{"prompt":"a normal prompt"}`), &out)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	...
	// additionalContext contains the resume context
	...
	// On the second call it is not injected (because the pending flag is gone)
...
// TestUserPromptInjectPolicy_ResumeContextCleanup verifies the flag and context file are removed after injection.
...
// TestUserPromptInjectPolicy_WorkModeWarning verifies the work-mode warning is injected only once.
func TestUserPromptInjectPolicy_WorkModeWarning(t *testing.T) {
	...
	// First call: the warning is injected
	var out bytes.Buffer
	...
	// Second call: the warning is not injected (warned flag present)
	var out2 bytes.Buffer
...
func TestDetectIntent(t *testing.T) {
	tests := []struct {
		...
		{"look up the definition of this symbol", "semantic"},
		{"add a variable", "semantic"},
		{"refactor this please", "semantic"},
		{"hello there", "literal"},
...
// TestSanitizeResumeContext verifies the sanitization processing.
func TestSanitizeResumeContext(t *testing.T) {
	tests := []struct {
		name  string
		input string
		want  string // when "", perform a "does not contain" check
		notIn string
	}{
		{
			name:  "normal text",
...
// TestResumeMaxBytesEnv verifies the byte limit set by the environment variable.

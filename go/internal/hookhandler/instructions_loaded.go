...
// approveOutput is a {"decision":"approve","reason":"..."} response.
...
//
// Called on the InstructionsLoaded event, it does the following:
// 1. Records the event to .claude/state/instructions-loaded.jsonl
// 2. Lightly verifies the existence of hooks.json
//
...
func HandleInstructionsLoaded(in io.Reader, out io.Writer) error {
	...
	// Resolve PROJECT_ROOT (CWD field first, then env, then pwd).
	...
	// Resolve event_name (hook_event_name first, event_name fallback).
	...
	// Lightly verify the existence of hooks.json.

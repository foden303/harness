...
// assertElicitationDecision is a helper that verifies the decision and reason in the output JSON.
...
func TestElicitationHandler_NormalSession_Approve(t *testing.T) {
	// HARNESS_BREEZING_SESSION_ID unset -> normal session -> approve
...
func TestElicitationHandler_BreezingSession_Deny(t *testing.T) {
	// HARNESS_BREEZING_SESSION_ID is set -> Breezing -> deny
...
func TestElicitationHandler_FallbackFields(t *testing.T) {
	// Test for server_name and id fallback
	t.Setenv("HARNESS_BREEZING_SESSION_ID", "")

	dir := t.TempDir()
	h := &ElicitationHandler{ProjectRoot: dir}

	// No mcp_server_name -> use server_name

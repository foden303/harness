...
func TestCleanupHandler_DeletesTempFiles(t *testing.T) {
	...
	// verify that the temporary files were deleted
	for _, name := range tempFiles {
	...
	// verify that session.json is preserved
...
func TestCleanupHandler_InboxGlobPattern(t *testing.T) {
	...
	// create only the inbox-*.tmp pattern
	...
	// a file not matching inbox-*.tmp (should not be deleted)

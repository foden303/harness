...
// It ports the main functions of session-init.sh to Go:
//  1. lightweight init for subagents
//  2. session JSON initialization (session.json)
//  3. Plans.md task counting
//  4. a JSON response containing additionalContext
//
// shell version: scripts/session-init.sh
type InitHandler struct {
	// StateDir is the path of the .claude/state directory. When empty it is inferred from cwd.
	StateDir string
...
// sessionJSON is the schema of session.json (minimal).
...
// initResponse is the JSON output to the SessionStart hook.
...
// Handle reads the SessionStart payload from stdin, performs session
// initialization, and writes JSON containing additionalContext to stdout.
func (h *InitHandler) Handle(r io.Reader, w io.Writer) error {
	...
	// lightweight init for subagents (skip session.json operations)
	if inp.AgentType == "subagent" {
		return writeJSON(w, initResponse{
			HookSpecificOutput: initHookOutput{
				HookEventName:     "SessionStart",
				AdditionalContext: "[subagent] lightweight init complete",
			},
		})
	}

	// determine the project root and state directory
	...
	// create the state directory (with symlink check)
	if err := ensureStateDir(stateDir); err != nil {
		// continue even on error (still output the banner and Plans info)
		_ = err
	}

	// initialize session.json (when it does not exist or is stopped)
...
func (h *InitHandler) initSessionFile(stateDir string) error {
	...
	if data, err := os.ReadFile(sessionFile); err == nil {
		...
		if json.Unmarshal(data, &s) == nil {
			// leave as-is unless stopped/completed/failed
			switch s.State {
			case "stopped", "completed", "failed":
				// needs fresh initialization
			default:
				return nil
			}
		}
...
func buildPlansInfo(plansFile string) string {
	...
	wipCount := countMatches(plansFile, "cc:WIP", "pm:pending")
	todoCount := countMatches(plansFile, "cc:TODO")

	return fmt.Sprintf("Plans.md: in-progress %d / not-started %d", wipCount, todoCount)
}
...
func buildAdditionalContext(plansInfo string) string {
	...
	sb.WriteString("# [harness] Session Initialization\n\n")
	sb.WriteString(plansInfo + "\n")
	sb.WriteString("\n## Marker Legend\n")
	sb.WriteString("| Marker | Status | Description |\n")
	sb.WriteString("|---------|------|------|\n")
	sb.WriteString("| `cc:TODO` | not-started | Impl (Claude Code) scheduled to run |\n")
	sb.WriteString("| `cc:WIP` | in-progress | Impl is implementing |\n")
	sb.WriteString("| `cc:blocked` | blocked | waiting on dependency task |\n")
	sb.WriteString("| `pm:pending` | requested by PM | in 2-Agent operation |\n")
	return sb.String()
}
...
// Returns an error if it is a symlink.
...
// writeFileAtomic writes a file atomically via a temporary file.

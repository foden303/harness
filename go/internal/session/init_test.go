...
func TestInitHandler_Subagent(t *testing.T) {
	// lightweight init for subagents
	h := &InitHandler{}
...
`
	...
	// WIP=1 (cc:WIP) + pm:pending=1 → in-progress 2
	// TODO=2
	if !strings.Contains(ctx, "in-progress 2") {
		t.Errorf("expected in-progress 2 in context, got %q", ctx)
	}
	if !strings.Contains(ctx, "not-started 2") {
		t.Errorf("expected not-started 2 in context, got %q", ctx)
	}
}

func TestInitHandler_SymlinkSessionFile(t *testing.T) {
	// a symlink is a security error (the error is ignored and a response is returned)
	dir := t.TempDir()
	stateDir := filepath.Join(dir, "state")
	...
	var out bytes.Buffer
	// no error; a response is returned
...
func TestCountMatches(t *testing.T) {
	...
	tests := []struct {
		...
		{[]string{"cc:TODO"}, 1},
		{[]string{"pm:pending"}, 1},
		{[]string{"cc:WIP", "pm:pending"}, 2},
		{[]string{"cc:WIP", "cc:TODO", "pm:pending"}, 3},
		{[]string{"cc:done"}, 1},

...
// On session end it appends to session-log.md and records end info in session.json.
//
// shell version: scripts/session-summary.sh
type SummaryHandler struct {
	// StateDir is the state directory path. When empty it is inferred from cwd.
	StateDir string
	// MemoryDir is the .claude/memory path. When empty, projectRoot/.claude/memory is used.
	MemoryDir string
...
type summarySessionData struct {
	...
	// raw JSON (for adding fields)
	raw map[string]interface{}
}

// Handle reads the Stop payload from stdin and writes the
// session-end summary to session-log.md.
func (h *SummaryHandler) Handle(r io.Reader, w io.Writer) error {
	...
	// update session.json (record end info)
	...
	// write the summary to stdout (only if there are changes)
...
// sessionLogEntry is an entry written to session-log.md.
...
// readSessionData reads the required information from session.json.
...
// readWIPTasks reads WIP/pending tasks from Plans.md.
func (h *SummaryHandler) readWIPTasks(plansFile string) []string {
	...
	for scanner.Scan() {
		line := scanner.Text()
		if strings.Contains(line, "cc:WIP") || strings.Contains(line, "pm:pending") {
...
func (h *SummaryHandler) appendSessionLog(logFile string, entry sessionLogEntry) error {
	...
	fmt.Fprintf(f, "\n### Handoff to Next Session (optional)\n")
	if len(entry.WIPTasks) > 0 {
		fmt.Fprintf(f, "\n**Plans.md WIP/pending (excerpt)**:\n\n```\n")
		for _, task := range entry.WIPTasks {
			fmt.Fprintln(f, task)
		}
		fmt.Fprintf(f, "```\n")
	} else {
		fmt.Fprintln(f, "- (add as needed)")
	}
...
func (h *SummaryHandler) appendEvent(eventLogFile, sessionFile, eventType, state, ts string) {
	...
	// read EventSeq from session.json and increment it
	seq := 0
...
// finalizeSessionFile records end info in session.json.
...
// archiveSession copies the session files to the archive directory.
...
// calcDurationMinutes computes the number of minutes from session start to now.
...
// stringField safely gets a string field from a map.
...
// boolField safely gets a bool field from a map.
...
// intField safely gets an int field from a map (JSON numbers are float64).
...
// bufioScanner wraps an io.Reader to provide line scanning.

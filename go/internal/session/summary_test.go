...
func TestSummaryHandler_AlreadyLogged(t *testing.T) {
	...
	// no output because double execution is prevented
	if out.Len() != 0 {
...
func TestSummaryHandler_WritesSessionLog(t *testing.T) {
	...
	// check that session-log.md was created
	...
	// check that memory_logged=true was set in session.json
...
func TestSummaryHandler_WritesWIPTasks(t *testing.T) {
	...
	// check that WIP tasks are included
...
func TestSummaryHandler_EnsureSessionLog(t *testing.T) {
	...
	// the second call does nothing (no overwrite)

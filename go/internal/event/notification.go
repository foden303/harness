...
// Fires when Claude Code issues a notification and records the event in a JSONL log.
//
// shell version: scripts/hook-handlers/notification-handler.sh
type NotificationHandler struct {
	// StateDir is the destination for the log file.
...
// and returns nothing on stdout.
// Errors are ignored and processing continues (so as not to affect Breezing's background operation).
func (h *NotificationHandler) Handle(r io.Reader, w io.Writer) error {
	...
	// Detect important notifications for Breezing's background Worker.
	// No output to stdout (log recording only)
	h.handleBreezingNotifications(notificationType, inp.AgentType)

	// The Notification hook needs no output (approve is implicit)
	_ = w
	return nil
}
...
// handleBreezingNotifications outputs important notifications for the Breezing
// background Worker to stderr (for debugging and observability).
func (h *NotificationHandler) handleBreezingNotifications(notificationType, agentType string) {
	...
	case "permission_prompt":
		// Worker cannot respond to a permission dialog
		fmt.Fprintf(os.Stderr,
			"Notification: permission_prompt for agent_type=%s\n", agentType)
	case "elicitation_dialog":
		// Input request from an MCP server (v2.1.76+).
		// A background Worker cannot respond to the Elicitation form
		fmt.Fprintf(os.Stderr,

...
type PostCompactHandler struct {
	// StateDir specifies the location of the snapshot file.
	// When empty, ResolveStateDir(projectRoot) is used.
	StateDir string
	// PlansFile specifies the path to Plans.md.
	// When empty, projectRoot/Plans.md is used.
	PlansFile string
}
...
// Fields that can be null are received as pointer types to unmarshal JSON null correctly.
...
// handoffPlanCounts is plan count information.
...
// handoffContextReset is context-reset recommendation information.
type handoffContextReset struct {
	Recommended bool   `json:"recommended"`
	Summary     string `json:"summary"`
}

// handoffContinuity is continuity context.
...
func (h *PostCompactHandler) Handle(r io.Reader, w io.Writer) error {
	...
	// Check for existence of the structured handoff artifact
...
// Prefers the structured handoff artifact, falling back to the WIP summary.
...
// A detailed equivalent of get_structured_handoff_context in the bash version post-compact.sh.
func (h *PostCompactHandler) extractStructuredContext(path string) string {
	...
	if len(snap.FailedChecks) > 0 {
		...
		for _, c := range checks {
			...
			// If detail exists, append it as "check: detail".
			// riskNormalizeText only returns secondary when primary is empty,
			// so combine both here to avoid dropping the detail information.
			if c.Check != "" && c.Detail != "" {

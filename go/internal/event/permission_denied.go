...
// Records the denial event to telemetry and, in Worker mode, returns a
// notification to the Lead and a retry hint.
//
...
// permissionDeniedInput is the stdin JSON of the PermissionDenied hook.
...
// and then returns retry + systemMessage for a Worker.
// For non-Worker agents it returns approve.
func (h *PermissionDeniedHandler) Handle(r io.Reader, w io.Writer) error {
	...
	if h.isWorker(agentType) {
		notificationText := fmt.Sprintf(
			"[PermissionDenied] Worker's tool %s was denied in auto mode. "+
				"Reason: %s. Consider an alternative approach, or approve it manually if needed.",
...
// Returns true when it is "worker", "task-worker", or ends with ":worker".

...
// It sets harness environment variables using CLAUDE_ENV_FILE.
//
// shell version: scripts/hook-handlers/session-env-setup.sh
type SessionEnvHandler struct {
	// PluginRoot is the root directory to look for the version file.
	// When empty, it is obtained from the CLAUDE_PLUGIN_ROOT environment variable.
	PluginRoot string
}
...
type SessionEnvVars struct {
	...
	HarnessBreezingSessionID string // Not written out when empty
}

// Handle reads the SessionStart payload from stdin and writes the harness
...
func (h *SessionEnvHandler) Handle(r io.Reader, _ io.Writer) error {
	...
	// Read stdin, but SessionStart does not need tool_name
...
// writeEnvFile appends the harness environment variables to CLAUDE_ENV_FILE.

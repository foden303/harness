...
// makeInput is a helper that builds a HookInput with the given agent_id / agent_type.
...
func TestHandleStart_RegistersAgent(t *testing.T) {
	...
	if agent == nil {
		t.Fatal("agent is not registered after HandleStart")
	}
...
func TestHandleStart_IsIdempotent(t *testing.T) {
	...
	if err := tracker.HandleStart(input); err != nil {
		t.Fatalf("first HandleStart error: %v", err)
	}
	// Second call: must be idempotent (no error).
	if err := tracker.HandleStart(input); err != nil {
		t.Fatalf("second HandleStart error: %v", err)
	}
...
func TestStatus_DurationIsPositive(t *testing.T) {
	...
	// Wait a bit before fetching Status.
...
func TestStatus_StateReflectsLatestTransition(t *testing.T) {
	...
	// Immediately after Start it is RUNNING.
...
func TestTracker_StateMachineConnectedToRecovery(t *testing.T) {
	...
	// Verify that SM and Recovery are connected:

...
//
// Resolution order:
//  1. Read plansDirectory from .harness.config.yaml at projectRoot
...
// .harness.config.yaml without requiring a YAML dependency.
...
func readPlansDirectoryFromHarnessConfig(projectRoot string) string {
	const configFile = ".harness.config.yaml"
